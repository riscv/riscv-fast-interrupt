...
if (mstatus.MPIE == 1) {
    ...
    loop {
        ...
        disable interrupts
        restore mpistatus
        write mpistatus.pithreshold to mithreshold /* restore threshold */
    }
}
...
fast_interrupt_dispatcher:
    ...
    sr      PISTATUS, 1*REGSZ, SP_REG

    csrr    EPC,      mepc
    csrr    PISTATUS, CSR_MPISTATUS
...
dispatch_exit:
    ...
    csrw    mepc, EPC
    csrw    CSR_MPISTATUS, PISTATUS

    lr      EPC,      0*REGSZ, SP_REG
    lr      PISTATUS, 1*REGSZ, SP_REG
    addi    SP_REG, SP_REG, (2*REGSZ)
    mret
----
