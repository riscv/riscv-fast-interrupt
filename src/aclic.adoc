...
While executing the ipush instruction, a hart may optionally check for higher priority pending and enabled interrupts at the current privilege mode, and switch to fetching a higher priority handler instead.
If such an interrupt exists, the hart may abort executing the ipush instruction and trigger the following actions:
  - Update the `xcause.Exception Code` to the corresponding interrupt id.
  - Perform the interrupt handler fetch.
  - Start executing the interrupt handler, potentially skipping portions of ipush that have been executed by the aborted ipush.

There will be a point of no return during the ipush instruction.
Specifically, once the interrupt pending bit is cleared, the ipush instruction must complete for the given interrupt source.
