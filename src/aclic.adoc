...
The field {mipreemptcfg}.{nipbits} defines how many most-significant priority bits participate in nested interrupt preemption grouping.
Interrupts whose priorities differ only in less-significant bits cannot preempt each other.
The highest legal value of {mipreemptcfg}.{nipbits} is IPRIOLEN.
...
----
NIPPRIO_MASK = ~(2^(mipreemptcfg.nipbits) - 1)
----

NOTE: For example, if {mipreemptcfg}.{nipbits}=4, 
NIPPRIO_MASK=11110000, i.e. the four MSBs are participating in interrupt preemption.

{fibits} defines the number of low-order priority bits used to form a contiguous fast-interrupt priority range from 0 through FITHRESHOLD.
This definition is intentionally orthogonal to {nipbits}.
The highest legal value of {mipreemptcfg}.{fibits} is IPRIOLEN.

NOTE: In the intended use-case, {mipreemptcfg}.{fibits} will be configured to values smaller than {mipreemptcfg}.{nipbits}.
This selects a subset of preemption groups as fast-interrupts.

[source,subs="attributes"]
----
FITHRESHOLD = (2^mipreemptcfg.fibits) - 1
----
...
NOTE: 
E.g. expanding on the example above, if {mipreemptcfg}.{fibits} = 2, then FITHRESHOLD = 00000011.
...
{sipreemptcfg} is a WARL register used to configure the behavior of preemption at S-mode.
...
[source]
----
(IPRIO & NIPPRIO_MASK) >= (xithreshold.iprio & NIPPRIO_MASK)
----
