...
writable bit in mip. When supervisor mode is not implemented, registers mvien and mvip do not exist."
 +
Further, resource-constrained implementations of these extensions are expected to implement the {iprio} array of major interrupts as read-only zeros.
...
. The source register bits [16:8] are written to {mithreshold}.{iprio}
...
. The source register bits [16:8] are written to {sithreshold}.{iprio}
. The reported interrupt identity is claimed by clearing its pending bit if possible.
...
{xithreshold}.{iprio} is set to {xtopsi}.{IPRIO}.
