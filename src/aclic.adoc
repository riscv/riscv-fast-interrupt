...
Interrupt handler latency and jitter are reduced by allowing the context save to happen speculatively in parallel with the vector table and handler fetch.

NOTE: In Harvard architectures, the data bus is idle while the vector table and handler fetch are happening.
