...
{MIID} is equivalent to the minor interrupt identity of `i`.
Else, if a major interrupt `i` is the highest-priority pending-and-enabled interrupt,
{MIID} is the two's complement representation of the negated major interrupt identity of `i`.
Else, if no interrupt is pending-and-enabled, {MIID} is 0.
