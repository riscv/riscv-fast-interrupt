...
Faults that occur during the read of interrupt jump table entries may not be recoverable.
If an unrecoverable load page fault can occur during the interrupt jump table read,
it is recommended to configure {xtvec}.{mode} to `10`, if interrupt vectoring is desired.

If an exception trap occurs during the interrupt jump table entry read,
the following holds true:
Depending on where the second trap is handled,
the corresponding exception pc ({xepc} or {mnepc} in case of an unexpected exception in M-mode with Smdbltrp) value is set to the exception pc of the first trap.
In addition, {xtval} is either set to zero or written with the faulting address.

NOTE: For simpler systems, we do not require that {xtval} is written
with the faulting address.  For systems with demand paging, {xtval}
...
Specifically, saving registers x10-x15 can cause faults that lead to synchronous exceptions.

NOTE: It is not recommended to use this extension,
if faults can occur during this sequence that are not recoverable.

If an exception trap occurs during the hardware stacking of an interrupt trap,
the following holds true:
Depending on where the second trap is handled,
the corresponding exception pc ({xepc} or {mnepc} in case of an unexpected exception in M-mode with Smdbltrp) value is set to the exception pc of the first trap.
In addition, {xtval} is either set to zero or written with the faulting address.
