...
Additionally, it provides interrupt vectoring capabilities for external interrupts.
...
table entry for the associated interrupt (table pointed to the new {xivt} CSR),
...
A new base address for the vector table is introduced.

The WARL XLEN-bit {xivt} CSR
holds the base address of the interrupt vector table.
The address held in {xivt} is aligned on a 64-byte or greater power-of-two boundary.
...
{xivt} values at different privilege levels may be configured to the same values, respectively.

{xivt}.{SHAMT} configures an additional shift amount that is applied to a vector table offset.
...
where:
  ...
  if VEN = 1
    handler = entry & VTMASK
  else
    handler = xtvec[XLEN-1:2]<<2
----
...
This SW dispatcher may access the table pointed to by {xivt} to retrive the interrupts handler address.
...
NOTE: If {xivt} is configured such that multiple privilege levels
...
that interrupts trapping to the SW dispatcher have the LSB of their interrupt vector table entry zero.
Interrupts using a fast handler must have the LSB of their vector table entry set to one.
...
==== Additional bit in {xiconfig} CSR

The {xiconfig} CSR is changed by adding a bit to control
if synchronous exception hardware vectoring is active.

The following bits of {xiconfig} are allocated to control exception hardware vectoring:

[source]
----
xiconfig fields
 bits        description
 7           sehv (if Ssehv is implemented)
 6           mehv (if Smehv is implemented, reserved in siconfig)
----
...
When {xiconfig}.{xehv} is clear, the exception trap behavior is unchanged.

When {xiconfig}.{xehv} is set, the synchronous exceptions trap to the following entry point:
