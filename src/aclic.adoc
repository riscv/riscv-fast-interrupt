...
:sipreemptcfg: pass:q[``sipreemptcfg``]
:xipreemptcfg: pass:q[``xipreemptcfg``]
:mnipbits: pass:q[``mnipbits``]
:snipbits: pass:q[``snipbits``]
:xnipbits: pass:q[``xnipbits``]
:spreempten: pass:q[``spreempten``]
:mpreempten: pass:q[``mpreempten``]
:xpreempten: pass:q[``xpreempten``]
:mfie: pass:q[``mfie``]
:sfie: pass:q[``sfie``]
:xfie: pass:q[``xfie``]
:mpfie: pass:q[``mpfie``]
:spfie: pass:q[``spfie``]
:xpfie: pass:q[``xpfie``]
:mfibits: pass:q[``mfibits``]
:sfibits: pass:q[``sfibits``]
:xfibits: pass:q[``xfibits``]
...
The extensions enable higher-priority interrupts to preempt lower-priority interrupts in a nested fashion.
...
----
 ...
 (changed) 0x35C   mtopei       Top external interrupt
 (changed) 0xFB0   mtopi        Top interrupt
----
...
mipreemptcfg fields
 ...
 15:12       sfibits (if Ssnip is implemented)
 11:8        mnipbits
 7:4         mfibits
 3           spreempten (if Ssnip is implemented)
 2           mpreempten
 1:0         (reserved)
----
...
If Ssnip is implemented, the fields {snipbits}, {sfibits}, and {spreempten} of {mipreemptcfg}
control nested interrupt preemption behavior for supervisor level.

The supervisor-level CSR {sipreemptcfg} is a subset of {mipreemptcfg}.
Reading or writing a field of {sipreemptcfg} reads or writes the corresponding field of {mipreemptcfg}.

The fields {mpreempten} and {spreempten} enable this extension at M-mode and S-mode, respectively.
Enablement of the extension refers to activating the modified operative behavior specified in <<op-threshold>>.

If Ssnip is not implemented, then {snipbits}, {sfibits}, and {spreempten} are read-only 0.

The field {mipreemptcfg}.{mnipbits}/{snipbits} defines how many most-significant priority bits participate in nested interrupt preemption grouping.
Interrupts whose priorities differ only in less-significant bits cannot preempt each other.
The highest legal value of {mipreemptcfg}.{mnipbits}/{snipbits} is IPRIOLEN.
...
----
NIPPRIO_MASK = ~(2^(mipreemptcfg.xnipbits) - 1)
----

NOTE: For example, if {mipreemptcfg}.{mnipbits}=4, 
NIPPRIO_MASK=11110000, i.e. the four MSBs are participating in interrupt preemption.

{mfibits} and {sfibits} define the number of low-order priority bits used to form a contiguous fast-interrupt priority range from 0 through FITHRESHOLD.
This definition is intentionally orthogonal to {xnipbits}.
The highest legal value of {mipreemptcfg}.{mfibits}/{sfibits} is IPRIOLEN.

NOTE: In the intended use-case, {mipreemptcfg}.{xfibits} will be configured to values smaller than {mipreemptcfg}.{xnipbits}.
This selects a subset of preemption groups as fast-interrupts.

[source,subs="attributes"]
----
FITHRESHOLD = (2^mipreemptcfg.xfibits) - 1
----

An interrupt source is considered a fast interrupt source depending on its {IPRIO} value, if the following condition holds:
...
where {FITHRESHOLD} at mode `x` is defined as:
...
NOTE: E.g. expanding on the example above, if {mipreemptcfg}.{mfibits} = 2, then FITHRESHOLD = 00000011.
In this case, only interrupts with {IPRIO}=0,1,2,3 are considered fast interrupts.
In AIA, IPRIO = 0 may represent the highest-priority class for certain interrupts using default priority ordering.
Such interrupts are therefore considered fast interrupts whenever the configured FITHRESHOLD includes zero.

NOTE: The fast interrupt threshold is defined this way to simplify the logic to determine,
if an interrupt is considered a fast interrupt.
No comparator is needed this way, and the determination can be done through a bitwise and-operation.
...
mistatus
 ...
 26         mpfie             Previous fast interrupts enable
 25:21      (reserved)
 ...
 6:2        (reserved) 
 1          mie               Interrupt enable, alias of mstatus.mie
 0          mfie              Fast interrupts enable
----

The bit {mfie} (Fast Interrupt Enable) controls if fast interrupts can trap when the corresponding {mie} bit is zero.

When {mie} is one, {mfie} has no effect.
When the {mfie} bit is set to 0 by an explicit CSR write, the {mie} bit is cleared to 0.

===== Changed top interrupt ({mtopi}) and top external interrupt ({mtopei}) CSRs

The CSRs {mtopi} and {mtopei} are enhanced
for efficient operation in combination with the thresholding operation
introduced for supporting efficient nested preemption.

The CSRs are modified as follows when {mipreemptcfg}.{mpreempten} is one:

When reading from {mtopi} or {mtopei}, and no interrupt is pending and enabled at machine level,
the {IPRIO} field returns {mithreshold}.{iprio}.
...
As {mithreshold}.{iprio} is IPRIOLEN+1 wide, the format of {mtopi} and {mtopei} is adjusted to:
...
----
 ...
 (changed) 0x15C   mtopei       Top external interrupt
 (changed) 0xDB0   mtopi        Top interrupt
----
...
{sipreemptcfg} is a WARL register that is a subset of {mipreemptcfg}.
The implemented fields of {sipreemptcfg} are aliases of the corresponding fields of {mipreemptcfg}.

The field {spreempten} enables this extension at S-mode.
The fields {snipbits} and {sfibits} configure nested interrupt preemption grouping and fast-interrupt classification for S-mode, respectively.
...
sistatus
 ...
 26         spfie             Previous fast interrupts enable
 25:21      (reserved)
 ...
 7          psppush           Previous stack pointer push, mirror of sspcs.ppush, if Sscsps is implemented
 6:2        (reserved) 
 1          sie               Interrupt enable, alias of sstatus.sie
 0          sfie              Fast interrupts enable
----

The supervisor {sstatus} register has only a single `spp` bit (to
indicate user/supervisor).

The field {sfie} controls if fast interrupts can trap when the corresponding {sie} bit is zero.

When {sie} is one, {sfie} has no effect.
When the {sfie} bit is set to 0 by an explicit CSR write, the {sie} bit is cleared to 0.
...
----
sithreshold
 9          sien        Alias of ~sstatus.sie
 8:0        iprio[8:0]  Interrupt-enable threshold
----

The field {sithreshold}.{iprio} implements exactly IPRIOLEN+1 bits.

===== Changed top interrupt ({stopi}) and top external interrupt ({stopei}) CSRs

The CSRs {stopi} and {stopei} are enhanced
for efficient operation in combination with the thresholding operation
introduced for supporting efficient nested preemption.

The CSRs are modified as follows when {sipreemptcfg}.{spreempten} is one:

When reading from {stopi} or {stopei}, and no interrupt is pending and enabled at machine level,
the {IPRIO} field returns {sithreshold}.{iprio}.

NOTE: Returning mithreshold.iprio as {IPRIO} field allows software to use this priority as threshold for spurious interrupts handlers.
If it would return zero, software would need special handling of spurious interrupts to prevent writing 0 to the threshold register.

As {sithreshold}.{iprio} is IPRIOLEN+1 wide, the format of {stopi} and {stopei} is adjusted to:
...
{mithreshold}.{iprio} resets to zero.
...
When {xipreemptcfg}.{xpreempten} is one,
...
[source]
----
IPRIO >= (xithreshold.iprio & NIPPRIO_MASK)
----
...
[NOTE]
====
The highest priority value of {IPRIO} is 0.
...
[NOTE]
====
The lowest priority value of {IPRIO} is 255.
...
If {xfie} is one, an interrupt can cause a trap irrespective of {xie}
if the interrupt is classified as a fast interrupt according to the configured value of {FITHRESHOLD}.
...
Also, the current value of {xistatus}.{xfie} is written into {xistatus}.{xpfie}, and then {xistatus}.{xfie} is set to zero.
...
{xret} sets {xistatus}.{xfie} to {xistatus}.{xpfie}.
...
===== Direct mode SW dispatcher

The first example modifies the example handler from AIA by adding support for nested preemption.
This handler assumes `mtvec.mode` = Direct (0).
In this mode, all traps have a common entry point, and thus all interrupts use a common SW dispatcher.
...
if (mstatus.MPIE == 1) {
    ...
    loop {
        i = read CSR mtopi
        IPRIO = (i & 0xFF)
        write IPRIO to mithreshold  /* raise threshold */
        id = i>>16
        enable interrupts
        call the interrupt handler for major interrupt id
        disable interrupts
        restore mistatus
        write mistatus.pithreshold to mithreshold /* restore threshold */
    }
}
...
The most important difference compared to not using Smnip is that interrupts are enabled before calling the interrupt handler.
While the interrupt is handled, a pending-and-enabled higher-priority interrupt can preempt the currently running interrupt handler.

It is necessary to raise the threshold after reading {mtopi},
because a higher priority interrupt could have become pending after the trap was taken.

Restoring the threshold after the handler call returns
allows lower priority interrupts to be handled
without restoring and saving the processor registers.

In case an interrupt an interrupt ceases to be pending before {mtopi} is read,
this is a spurious interrupt.
It can occur for level-sensitive interrupts.
To handle those, a spurious interrupt handler needs to be installed for interrupt id 0.

===== Direct mode SW dispatcher including external interrupts

The handler needs to be extended for efficient handling of external interrupts,
if external interrupts are prioritized among each other.
The example below assumes that the top external interrupt can be read and claimed via {mtopei}.
...
if (mstatus.MPIE == 1) {
    ...
    loop {
        ...
        if (id == MEI) {
          i = read CSR mtopei
          eid = i>>16
          handler_addr = ei_handlers[eid]
        } else {
          handler_addr = i_handlers[id]
        }
        IPRIO = (i & 0xFF)
        write IPRIO to mithreshold  /* raise threshold */
        enable interrupts
        call the interrupt handler at handler_addr
        disable interrupts
        restore mistatus
        write mistatus.pithreshold to mithreshold /* restore threshold */
    }
...
===== Vectored mode SW dispatcher

This example further modifies the example handler by considering `mtvec.mode` = Vectored (1).
In this mode, some interrupts may share a common SW dispatcher,
while others (higher priority interrupts) are handled in dedicated handlers (referred to as fast interrupt handlers here).

In a system based on this example, fast interrupts must be able to preempt other interrupts.
Because the fast interrupt handlers are not guaranteed to comply with the psABI,
it must also be guaranteed that the common SW dispatcher does not dispatch fast interrupt handlers.

Any fast interrupt handler must be configured to a nested interrupt priority that is higher than interrupt using the common SW dispatcher.

In this example, it is assumed that exceptions have an entry point on traps that is different from interrupts.

[,c]
----
save mepc, mistatus
enable fast interrupts (mfie = 1)
save processor registers
...
loop {
    ...
    call the interrupt handler for major interrupt id
    disable all interrupts (mie = 0 -> mfie = 0)
    restore mistatus
    write mistatus.pithreshold to mithreshold /* restore threshold */
    enable fast interrupts (mfie = 1)
}
...
restore processor registers
disable fast interrupts (mfie = 0)
restore mepc, mistatus
return from trap
----
(This example can be further optimized, but with an increase in complexity.)

In this example, fast interrupts are enabled for an extended period.
After saving the minimal state needed to resume after preemption,
fast interrupts are being enabled.
Enabling fast interrupts has the highest urgency.

This allows fast interrupts to preempt the dispatcher.
Because of that, {mtopi} can never report a fast interrupt.

Interrupts can still be globally enabled before calling the interrupt handler,
to ensure that other interrupts, that are not fast interrupts, 
but have a higher nested interrupt preemption priority can preempt the currently executing handler.

While restoring the threshold before the next loop iteration,
all interrupts have to be disabled.
As writing zero to {mie} also clears {mfie},
fast interrupts need to be enabled afterwards.

[[smnip-handler-vectored-external]]
===== Vectored SW dispatcher including external interrupts

This section builds upon the two handler examples introduced previously.
...
(draw-box "XLEN-2" {:span 26 :borders {}})
...
If the LSB of the vector table entry is 1, and the fetch is successful,
the hart clears the corresponding interrupt-pending bit if possible.
