...
| Smaplicind   | Indirect CSR access to APLIC registers at Machine level
| Ssaplicind   | Indirect CSR access to APLIC registers at Supervisor level
| Smictrl      | Interrupt control interface based on IMSIC at Machine level
| Ssictrl      | Interrupt control interface based on IMSIC at Supervisor level
...
== Fundamental components of ACLIC (Smaclic, Ssaclic)
...
but the same structures for configuration as the APLIC are used.

An ACLIC is made up of three components:

. A hart-local APLIC (Non-ISA)
. Indirect CSR access to APLIC registers (Smaplicind, Ssaplicind)
. Interrupt control interface based on IMSIC (Smictrl, Ssictrl)

ACLIC at machine level is enabled by Smaclic,
which is a group of extensions composed of Smaplicind and Smictrl,
and implies the implementation of a hart-local APLIC.

ACLIC at supervisor level is enabled by Ssaclic,
which depends on Smaclic,
and is a grop of extensions composed of Ssaplicind and Ssictrl.

=== Hart-local APLIC (Non-ISA)

The APLIC was designed as a multi-hart interrupt controller.
Here, the APLIC is used tightly coupled with a single hart.
This leads to a set of simplifications of the APLIC which is described here.
...
Endianess is controlled exclusively by the setting configured in the {xstatus} register at the hart.

Further simplifications are:

* The Child Index field in the sourcecfg[i] register is always '1' when D=1 (i.e. delegation to child domain). This is only possible in Machine mode. In Supervisor mode, D=0, as there is no child domain.
* As the ACLIC is not MSI capable, xmsiaddrcfg[h] and genmsi registers are not required.
* Interrupt delivery control (IDC) structure is not needed, as its functionality is taken over by hart-local CSRs.
* The hart index inside the target registers is read-only zero, as only one hart is supported.

APLIC registers or fields not required here shall and are read-only zero.

=== Indirect CSR access to APLIC registers (Smaplicind, Ssaplicind)

Smaplicind depends on the Smcsrind extension, and Ssaplicind depends on Sscsrind.

Indirect CSR access to APLIC registers is provided to subsume
...
No dedicated access to interrupt pending or enable bits is provided in this extension.
Access to these bits is expected to be provided in other extensions.

To have full control over the necessary APLIC registers, the following indirect CSR access is added.
The Smaplicind extension gives machine-level access to the root domain (x=m),
and Ssaplicind gives supervisor-level access to the child domain (x=s)
...
The Smictrl and Ssictrl depend on Smaia and Ssaia, respectively.
...
This does include the IMSIC register file,
which is repurposed to access the interrupt pending and enable bits of the hart-local APLIC.
...
If Smictrl or Ssictrl extensions are present, {eidelivery} is fixed to 0x20000000.
...
NOTE: Providing access to APLIC pending and enable bits at the hart-level has several advantages over using using an IMSIC with interrupt delivery from an APLIC
...
Interrupt handling throughput is increased by allowing tail-chaning of interrupts or synchronous exceptions, skipping the context restore and save in between servicing two handlers.
...
When executing an {ipopxret} instruction the context save during the trap is restored and a return is executed.
...
Finally, it should generate an {ixpopret} instruction at the end of the function instead of an {xret}.
...
While executing the ipopxret instruction,
...
If such an interrupt exists, the hart may abort executing the ipopxret instruction and trigger the following actions:
  ...
  - Start executing the interrupt handler, while skipping the portions of context restore that have not yet been executed by the aborted ipopxret.

Explicitly, {xepc}, {xpistatus} remain unchanged.

NOTE: In the typical case of tail chaining, the check for another pending enabled interrupt is at the start of the ipopxret instruction.
In this case, the entire context save and restore sequence can be skipped.

=== Fault handling

If synchronous exceptions cannot occur during the context save/restore segments,
