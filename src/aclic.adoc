...
mipreemptcfg fields
 bits        description
 XLEN-1:17   (reserved)
 16:13       snipbits (if Ssnip is implemented)
 12:9        sfibits (if Ssnip is implemented)
 11:8        mnipbits
 7:4         mfibits
...
If Ssnip is not implemented, then {spreempten} is read-only 0.
...
NOTE: 
E.g. expanding on the example above, if {mipreemptcfg}.{mfibits} = 2, then FITHRESHOLD = 00000011.
