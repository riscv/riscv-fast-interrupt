...
It is the responsibilty of the programmer
...
To decide if a stack pointer swap is required, these instructions evaluate dedicates status bits.
...
E.g., it allows sharing a interrupt jump table, where M-mode has read/write permissions and S-mode has read-only permissions.
...
Additional, M-mode has write permissions to the interrupt jump table.
...
it is recommended configure {xtvec}.{mode} to `10`, if interrupt vectoring is desired.

If an exception trap occurs during the interrupt interrupt jump table entry read,
...
This is not an architectural requirment, but a possible implementation choice.
...
Specifically, saving registers x10-x15 can cause a faults that lead to synchonous exceptions.
