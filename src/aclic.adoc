...
:preemptbits: pass:q[``preemptbits``]
...
mipreemptcfg fields
 ...
 2           mpreempten
 1           sfie (if Ssnip is implemented)
 0           mfie
----
...
The bit {xfie} (Fast Interrupt Enable) controls if fast interrupts can trap when the corresponding {xie} bit is zero.

When {xie} is one, {xfie} has no effect.
When the {xfie} bit is set to 0 by an explicit CSR write, the {xie} bit is cleared to 0.

The field {mipreemptcfg}.{preemptbits} defines the number of bits considered in preemption.
The highest legal value of {mipreemptcfg}.{preemptbits} is IPRIOLEN.
...
----
NIPPRIO_MASK = ~(2^(mipreemptcfg.preemptbits) - 1)
----

NOTE: For example, if {mipreemptcfg}.{preemptbits}=4, 
NIPPRIO_MASK=11110000, i.e. the four MSBs are participating in interrupt preemption.

Which subset of the interrupts is considered fast interrupts is controlled by {fibits}.
The highest legal value of {mipreemptcfg}.{fibits} is IPRIOLEN.

NOTE: In the intended use-case, {mipreemptcfg}.{fibits} will be configured to values smaller than {mipreemptcfg}.{preemptbits}.
...
NOTE: 
E.g. expanding on the example above, if {mipreemptcfg}.{fibits}, then FITHRESHOLD = 00000011.
In this case, only interrupts with {IPRIO}=1,2,3 are consider fast interrupts.
...
mithreshold
 ...
 7:0        iprio[7:0]  Previous interrupt-enable threshold
----
...
mistatus
 ...
 26         pmfie             Previous fast interrupts enable
 25:20      (reserved)
...
sistatus
 ...
 26         psfie            Previous fast interrupts enable
 25:20      (reserved)
 19:18      VS[1:0]           Vector extension status, mirror of mstatus.VS
 17:16      FS[1:0]           Floating-point extension status, mirror of mstatus.FS
 15:8       pithreshprio[7:0] Previous mithreshold.iprio value
...
the {IPRIO} field returns {sithreshold}.
...
{xistatus}.{pithreshold} is unchanged.
