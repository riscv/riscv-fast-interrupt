...
.External interrupt hardware vector table layout for `{xtvec}.MODE=10`
...
| `-4 * N`
| Jump to the handler for the corresponding major interrupt identity N.

| `-4 * 11`
| Jump to the handler for a machine external interrupt.

| `-4 * 9`
| Jump to the handler for a supervisor external interrupt.

| `-4 * 7`
| Jump to the handler for a machine timer interrupt.

| `-4 * 5`
| Jump to the handler for a supervisor timer interrupt.

| `-4 * 3`
| Jump to the handler for a machine software interrupt.

| `-4 * 1`
| Jump to the handler for a supervisor software interrupt.

| `+0`
| Jump to the common synchronous exception handler and interrupt handler

| `+4 * 1`
| Jump to the handler for external interrupt minor identity 1.

| `+4 * 2`
| Jump to the handler for external interrupt minor identity 2.

| `+4 * N`
| Jump to the handler for external interrupt minor identity N.
...
The CSRs {mtopsi} is provided for efficiently handling major and external interrupts
in a scheme, where interrupts are dispatched by software.
It is designed to manage the interrupt threshold when claiming interrupts,
to enable efficient nested preemption.

A read of {mtopsi} returns information about the highest-priority pending-and-enabled interrupt for machine level, in this format:
...
The CSRs {stopsi} is provided for efficiently handling major and external interrupts
in a scheme, where interrupts are dispatched by software.
It is designed to manage the interrupt threshold when claiming interrupts,
to enable efficient nested preemption.

A read of {stopsi} returns information about the highest-priority pending-and-enabled interrupt for supervisor level, in this format:
...
. The the source registers value is written to {sithreshold}.{iprio}
. The reported interrupt identity is claimed by clearing its pending bit if possible.
