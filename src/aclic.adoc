...
Which subset of the interrupts is considered fast interrupts is controlled by {fithresh}.
The highest legal value of {mipreemptcfg}.{fithresh} is IPRIOLEN.

NOTE: In the intended use-case, {fithresh} will be configured to values smaller than {preemptmsk}.
This selects a subset of preemption groups as fast-interrupts.

[source,subs="attributes"]
----
FITHRESHOLD = 2^mipreemptcfg.fithresh
----
...
mistatus 
 ...
 16:9       pithreshold[7:0] Previous interrupt-enable threshold
 8          psppush          Previous stack pointer push, mirror of mspcs.ppush, if Smcsps is implemented
 7:2        (reserved) 
 0          mfie             Fast interrupts enable
----

The bits {mfie} and {sfie} (Fast Interrupt Enable) controls if fast interrupts can trap when the corresponding {xie} bit is zero.
If Ssnip is not implemented, then {sfie} is read-only 0.

When {xie} is one, {xfie} has no effect.
When the {xfie} bit is set to 0 by an explicit CSR write, the {xie} bit is cleared to 0.

NOTE: The inclusion of mfie in bit 0 allows setting it with a CSRRSI instruction while also reading the CSR for the purpose of context saving.
This saves one instruction in the path to enabling fast interrupts.
...
the current value of {xithreshold} is written to {xistatus}.{pithreshold}.
...
{xret} sets {xithreshold} to {xistatus}.{pithreshold}.
...
if (mstatus.MPIE == 1) {
    ...
    loop {
        ...
        if (id == MEI) {
          i = read CSR mtopei
          if (i == 0) skip to next loop iteration
          eid = i>>16
...
#if __riscv_xlen == 64
  #define REGSZ 8
  .macro sr r,o,b; sd \r,\o(\b); .endm
  .macro lr r,o,b; ld \r,\o(\b); .endm
  .macro shxadd r,idx,b; sh3add \r,\idx,\b; .endm
#else
  #define REGSZ 4
  .macro sr r,o,b; sw \r,\o(\b); .endm
  .macro lr r,o,b; lw \r,\o(\b); .endm
  .macro shxadd r,idx,b; sh2add \r,\idx,\b; .endm
#endif

#define MEI_ID 11

#define CSR_MISTATUS     0x346
#define CSR_MITHRESHOLD   0x347
#define CSR_MIPREEMPTCFG  0x348
...
#define IRQ_ID      t1
#define CMP_TMP     t2
#define HANDLER_PTR t3

.section .text
.align 4
.globl fast_interrupt_dispatcher

fast_interrupt_dispatcher:
    addi    SP_REG, SP_REG, -(2*REGSZ)
    sr      EPC,      0*REGSZ, SP_REG
    sr      ISTATUS, 1*REGSZ, SP_REG

    csrr    EPC,      mepc
    csrr    ISTATUS, CSR_MISTATUS

    csrsi   CSR_MIPREEMPTCFG, 1

    call    __riscv_save

dispatch_loop:
    csrr    TOPI_VAL, mtopi
    beqz    TOPI_VAL, dispatch_exit

    srli    IRQ_ID, TOPI_VAL, 16
    li      CMP_TMP, MEI_ID
    bne     IRQ_ID, CMP_TMP, int_irq

ext_irq:
    csrr    TOPI_VAL, mtopei
    beqz    TOPI_VAL, dispatch_loop
    srli    IRQ_ID, TOPI_VAL, 16
    la      HANDLER_PTR, ei_handlers
    shxadd  HANDLER_PTR, IRQ_ID, HANDLER_PTR
    lr      HANDLER_PTR, 0, HANDLER_PTR

have_handler:
    csrw    CSR_MITHRESHOLD, TOPI_VAL

    csrsi   mie, 1
    jalr    HANDLER_PTR
    csrci   mie, 1

    csrw    CSR_MITHRESHOLD, ISTATUS
    csrsi   CSR_MIPREEMPTCFG, 1

    j       dispatch_loop

int_irq:
    la      HANDLER_PTR, i_handlers
    shxadd  HANDLER_PTR, IRQ_ID, HANDLER_PTR
    lr      HANDLER_PTR, 0, HANDLER_PTR
    j       have_handler
...
dispatch_exit:
    ...
    csrw    mepc, EPC
    csrw    CSR_MISTATUS, ISTATUS

    lr      EPC,      0*REGSZ, SP_REG
    lr      ISTATUS, 1*REGSZ, SP_REG
    addi    SP_REG, SP_REG, (2*REGSZ)
    mret
----
