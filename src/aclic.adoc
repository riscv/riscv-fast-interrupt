...
The two LSBs of this register contain the mode that the stack pointer is associated with.

An additional bit is allocated to indicate that the stack pointer in {xsp} belongs to privilege mode {x}.
...
[source]
----
if (xsp[2] != 0)             /* stack pointer in xsp belong to privilege mode x and shall be swapped */
    tmp = rs                 /* save current sp for writing later */
    rd = xsp & ~7            /* clear bits 2:0 of the stack pointer and write to sp */
    xsp = tmp                /* save original value of sp into xsp */
    xsp[1:0] = xtstatus.XPP  /* set LSBs to previous privilege mode */
----
...
[source]
----
if (xsp != 0) and (xsp[2] == 0) and (xstatus.XPP == xsp[1:0])
    tmp = xsp                /* save xsp as temporary value */
    xsp = rs | 4             /* write current stack pointer to xsp and assert bit 2 */
    rd = tmp & ~0xF          /* write original xsp value to stack pointer with low nibble cleared */
----
