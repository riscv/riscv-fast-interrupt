...
then access to the new CSRs (`macliciprio[k]`, `maclicsourcecfg[k]`, `sacliciprio[k]`, `saclicsourcecfg[k]`, `vsacliciprio[k]`, `vsaclicsourcecfg[k]`) by S-mode or a lower privilege mode
...
Anologously, if Sstateen is implemented,
bit ?? of hstateen0 controls less-privileged access to `vsacliciprio[k]`, `vsaclicsourcecfg[k]`.

NOTE: Clearing the corresponding control bit in hstateen0
allows a hypervisor to maintain full control over the priority assignment.
Any guest OS access will result in a virtual instruction exception, 
which can be handled by the hypervisor.
...
=== Changed and new CSRs at Hypervisor and VS level

[source]
----
       Number  Name         Description
 (NEW) 0x???   hpintstatus  Previous interrupt context at hypervisor level
 (NEW) 0x???   vspintstatus Previous interrupt context at virtual supervisor level
----

==== New Interrupt Status ({hpintstatus}) CSR
...
spintstatus
 ...
 30:29  (reserved)
    28  spp            Previous privilege mode, mirror of sstatus.spp
    27  spie           Previous interrupt enable, mirror of sstatus.spie
    26  spv            Supervisor previous virtualization mode, mirror of hstatus.spv
    25  spvp           Supervisor Previous Virtual Privilege, mirror of hstatus.spvp
    24  gva            Previous guest virtual address, mirror of hstatus.gva
 26:24  (reserved)
 23:16  hpnipprio[7:0] Previous nested interrupt preemption priority
 15:0   (reserved)
----

==== New Interrupt Status ({vspintstatus}) CSR

When V=1, {vspintstatus} substitutes for the usual {spintstatus}, so instructions that normally
read or modify {spintstatus} actually access {vspintstatus} instead.
...
vspintstatus
 ...
 XLEN-1 Interrupt      Interrupt=1, Exception=0, mirror of scause.Interrupt
 30:29  (reserved)
    28  spp            Previous privilege mode, mirror of vsstatus.spp
    27  spie           Previous interrupt enable, mirror of vsstatus.spie
 26:24  (reserved)
 23:16  spnipprio[7:0] Previous nested interrupt preemption priority
 15:0   (reserved)
----
...
If the trap is taken on an interrupt, {mintstatus}.{xnipprio} is updated to `nipprio[__i__]`.
...
then access to the new CSRs ({spintstatus}, {hpintstatus}, {vspintstatus}) by S-mode or a lower privilege mode
results in an illegal instruction exception,
except if the hypervisor extension is implemented,
and the conditions for a virtual instruction exception apply,
in which case a virtual instruction exception is raised
...
Anologously, if Sstateen is implemented,
bit 53 of hstateen0 controls less-privileged access to {vspintstatus}.
...
----
 ...
 (NEW) 0x108   seivt        S-mode External Interrupt-handler vector table base address
       0x205   vstvec       VS-mode Trap-handler base address / interrupt mode
 (NEW) 0x207   vsivt        VS-mode Interrupt-handler vector table base address
 (NEW) 0x208   vseivt       VS-mode External Interrupt-handler vector table base address
----
...
then the bit 53 (ACLIC) in mstateen0 is implemented.
If bit 53 (ACLIC) of a controlling mstateen0 CSR is zero,
then access to the new CSRs ({sivt}, {vsivt}) by S-mode or a lower privilege mode
results in an illegal instruction exception,
except if the hypervisor extension is implemented,
and the conditions for a virtual instruction exception apply,
in which case a virtual instruction exception is raised
...
Anologously, if Sstateen is implemented,
bit 53 of hstateen0 controls less-privileged access to {vsivt}.
...
The Smcsps extension depends on the Smnnip extension.
...
The Sscsps extension depends on the Ssnnip extension.
...
(NEW) 0x148   vssp         VS-mode stack pointer for conditional swap (WARL)
----
...
(draw-box "SWAP" {:span 2 :text-anchor "end" :borders {:top :border-unrelated :bottom :border-unrelated :left :border-unrelated}})
...
(draw-box "MODE" {:span 2 :text-anchor "end" :borders {:top :border-unrelated :bottom :border-unrelated :left :border-unrelated}})
(draw-box (text "(WARL)" {:font-weight "bold"}) {:span 3 :text-anchor "start" :borders {:top :border-unrelated :bottom :border-unrelated :right :border-unrelated}})
...
The `MODE` field contains the mode that the stack pointer is associated with.
Its encoding of modes is identical to the `XPP` field in {xstatus}.

An additional bit `SWAP` indicates that the stack pointer in {xsp} belongs to privilege mode `x`.
If found asserted, this indicates that the stack pointer should be swapped before using the stack.
...
To decide, if a stack pointer swap is required, these instructions evalute the two LSBs of the register, indication the mode.
...
then the bit 53 (ACLIC) in mstateen0 is implemented.
If bit 53 (ACLIC) of a controlling mstateen0 CSR is zero,
then access to the new CSRs ({ssp}, {vssp}) by S-mode or a lower privilege mode
results in an illegal instruction exception,
except if the hypervisor extension is implemented,
and the conditions for a virtual instruction exception apply,
in which case a virtual instruction exception is raised
...
Anologously, if Sstateen is implemented,
bit 53 of hstateen0 controls less-privileged access to {vssp}.
...
The stack pointer swap is conditionally executed,
if the privilege mode store with the stack pointer equals the current mode.
After swapping the stack pointer, previous privilege mode is written to the low bits of {xsp},
indicating that the correct stack pointer for the given mode is active.

The behavior of the operations is:

[source]
----
if (xsp.SWAP != 0)           /* stack pointer in xsp belong to privilege mode x and shall be swapped */
    tmp = rs                 /* save current sp for writing later */
    rd = xsp.SP << 4         /* Write the stack pointer with the low bits cleared */
    xsp = tmp                /* save original value of sp into xsp */
    xsp.MODE = xtstatus.XPP  /* set LSBs to previous privilege mode */
----
...
A swap is performed, if the stored stack pointer is not 0, the `SWAP` field is not set,
and the previous mode that will be returned to is equal to the mode stored with the stack pointer.
Additionally, it is checked if the previous nested interrupt preemption priority is the minimum value.
This indicates that we are not in a nested handler.

Behavior::

[source]
----
if (xsp != 0) and (xsp.SWAP == 0) and (xstatus.XPP == xsp.MODE) and (xpintstatus.xpnipprio == NIPPRIO_MAX)
    tmp = xsp.SP << 4        /* save xsp as temporary value */
    xsp.SP = rs[XLEN-1:4]    /* write current stack pointer to xsp */
    xsp.SWAP = 1             /* mark stack pointer as to be swapped */
