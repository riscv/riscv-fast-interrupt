...
----
XNIPPRIO_MASK = ~(2^(mipreemptcfg.preemptmsk) - 1)
xnipprio[i]   = iprio[i] & NIPPRIO_MASK
----
...
The Smcsps extension depends on the Smnip extension.
...
The Sscsps extension depends on the Ssnip extension.
...
To decide, if a stack pointer swap is required, these instructions evaluate the two LSBs of the register, indication the mode.
