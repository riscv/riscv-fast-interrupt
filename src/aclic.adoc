...
ACLIC implements subset of AIA APLIC configuration and control interface and allows maximum of two domains for Machine and
Supervisor modes respectively. Each domain may include:
...
ACLIC inherits valuable subset of the external interrupt configuration interface defined by RISC-V
AIA for Advanced Platform Level Interrupt Controller (APLIC).

NOTE: Entire ACLIC interface is optimized for access by interrupt identity number (IID) to
avoid unnecessary calculation of the bit positions.
...
| DM    |  3:2      |  RW     |  Delivery Mode (*WARL*)
|===

New APLIC delivery mode is defined in domaincfg.DM field:

[%autowidth,float="center",cols=">,<",grid=none,frame=none]
|===
...
Each implemented ACLIC interrupt has dedicated source configuration register which integrates
all parameters dedicated to this interrupt. The layout of the interrupt source configuration register
depends on value of delegation flag `D`, which is accessible in Machine domain configuration and is
always read-only zero in Supervisor domain configuration (assuming S-mode domain has no child).
...
To reduce registers addresses reservation the PRIO filed is moved from APLIC targetK[1..1023] registers.

The IE and IP fields are specific to ACLIC (reserved in case of APLIC) and provide read-only view
of the interrupt-pending and interrupt-enable bits replacing the pending and enable arrays
used by APLIC.

If delegation is enabled for the interrupt source K (sourcecfgK.D = 1, accessible only via M-domain
interface), then remaining fields of the sourcecfgK are read-only zeros in M-domain and have
extended layout in S-domain.

[%autowidth]
|===
| Field |  Position |  Access |  Description
 4+^| `sourcecfgK.D = 1`
| D = 1 |  10       |  RW     |  Machine domain interrupt source delegation is enabled
|===

NOTE: As the sourcecfgK.D is invisible in Supervisor domain, the effective value of the non-delegated source configuration is read-only zero.

==== Interrupt-Enable and Interrupt-Pending Control Registers

ACLIC implements subset of interrupt-pending and interrupt-enable control interface defined by RISC-V AIA for APLIC. In addition to set bit and clear bit by number there is read-only interrupt pending and interrupt-enable bits mirrored in the sourcecfgK source configuration register.
...
simplified as there is no need to construct APLIC-like pending/enable 32-bit values based
on per-interrupt delegation configuration.

==== Interrupt Delivery Control Registers

Like the APLIC in direct delivery mode, ACLIC provides the interrupt delivery control (IDC)
