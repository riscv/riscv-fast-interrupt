...
(draw-box "POP" {:span 2 :text-anchor "end" :borders {:top :border-unrelated :bottom :border-unrelated :left :border-unrelated}})
(draw-box (text "(WARL)" {:font-weight "bold"}) {:span 3 :text-anchor "start" :borders {:top :border-unrelated :bottom :border-unrelated :right :border-unrelated}})
...
In addition to the stack pointer, this register hold two additional fields, `PUSH` and `POP`.
These indicate, if a stack pointer swap is excepted during a push or pop sequence, respectively.
...
To decide, if a stack pointer swap is required, these instructions evalute the two LSBs of the register.
The `PUSH` bit indicates that the stack pointer in {xsp} shall be swapped during the push sequence of the next handler.

The `POP` bit indicates the the stack pointer shall be swapped back into {xsp}.
It is automatically asserted when swapping a stack pointer with cspspush.
...
The trap behavior is modified as follows:

When a trap is taken, {xsp}.`POP` is cleared.
...
[source]
----
if (xsp.PUSH == 1) and (xsp != 0)  /* stack pointer in xsp belong to privilege mode x and shall be swapped */
    ...
    xsp.POP = 1              /* Indicates the stack pointer needs to be swapped in a pop sequence */
----
