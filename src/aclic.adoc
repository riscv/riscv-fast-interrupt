...
When interrupts are delivered from an ACLIC, the IMSIC registers {eithreshold}, {eipk} and {eiek}
interact with the corresponding {ithreshold}, {setip}[_k_]/{in_clrip}[_k_], and {setie}[_k_]/{clrie}[_k_] of the connected APLIC domain.
...
while a write with the value 0 performans a write access with value 1 to the corresponding {in_clrip} or {clrie} register.
