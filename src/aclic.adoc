...
Indirect access to `acliciprio[k]` mirrors `target[k*4].iprio` up to `target[k*4+3].iprio`.

When XLEN = 64, only the even-numbered registers exist and each register controls the priority setting of eight interrupts.
Indirect access to `acliciprio[k]` mirrors `target[k*8].iprio` up to `target[k*8+7].iprio`.
...
If bit 58 of a controlling mstateen0 CSR is zero, then access to the CSRs reused from the IMSIC (eip[k] and eie[k])
...
The {xistatus}.{psppush} (Previous stack pointer push) bit indicates that the stack pointer shall be swapped back into {xspcs} during the pop sequence of the current handler.
...
The trap handler function address is read from `JTBASE+XLEN/8*vtoffset`.
...
The size of the stack frame created is set to XLEN,
