...
+
The tpush instruction writes the value of the xcause CSR or the interrupt identity into register a0.
...
| a1 | `xepc` | No Write | No Write
| a2 | `xcause.Interrupt` (0) | `xcause.Interrupt` (1) | `xcause.Interrupt` (1)
| a3 | `xtval` | No Write | No Write
| a4 | `xtval2` | No Write | No Write
| a5 | `xtinst` | No Write | No Write
|===
+
...
The values of xtval, xtval2, and xinst only need to be written to an argument register if the corresponding exception caused a write to the respective CSR.
+
`xcause.Interrupt` can be used to distinguish synchronous exceptions from interrupts in shared handlers.
+
As a consequence, the following prototypes could serve for an interrupt handler or an exception handler, respectively.
+
[source,c]
----
void interrupt_handler(uint16_t id) __attribute__((interrupt, prestacked("x12-x15")));
void exception_handler(uint16_t exception_code, uintptr_t epc, bool interrupt, uintptr_t tval, uintptr_t tval2, uintptr_t tinst) __attribute__((interrupt, prestacked("x15")));
----

Usually, interrupt handler will not require epc as an argument.
If the GPR holding epc is to be used, it will first be saved to the stack and restored later.
...
push:
  ...
  csrr a1, xepc             # Get xepc of interrupt context
  srli a2, a0, XLEN-1       # Isolate interrupt bit
  csrr a3, xpistatus        # Get xpistatus of interrupt context
  sr a3, OFFSET(sp)         # Save xpistatus
  bgez a0, handle_exc       # Handle synchronous exception
  csrr a0, xtopi            # Read major interrupt number
  li a4, xEI                # Write major identity of external interrupt
  beq a0, a4, handle_ei     # Handle external interrupt
...
handle_exc:
  csrr a3, xtval            # Get xcause of interrupted context
  csrr a4, xtval2           # Get xepc of interrupt context
  csrr a5, xtinst           # Get xpistatus of interrupt context
...
While this assembly sequence clobbers a3 to save xpistatus,
this is not a requirement. 
SW shall not make assumptions about value of registers, with the exception of the specified trap information.
...
This instruction requires {xepc} to be present in `a1`.

[source]
----
pop:
  csrrsi zero, xstatus, XIE # Disable interrupts
  lr a0, OFFSET(sp)         # Restore xpistatus
  csrw xepc, a1             # Write xepc back to CSR
  csrw xpistatus, a0        # Write xpistatus back to CSR
