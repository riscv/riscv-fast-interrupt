...
In addition to the stack pointer, this register hold two additional fields, `PUSH` and `PPUSH` (as in previous `PUSH`).
...
The `PPUSH` bit indicates the the stack pointer shall be swapped back into {xsp}.
...
When a trap is taken, {xsp}.`PPUSH` is set to {xsp}.`PUSH`.
...
A swap is performed, if the stored stack pointer is not 0, the `PUSH` field is not set,
