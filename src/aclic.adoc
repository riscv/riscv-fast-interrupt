...
the current value of {xithreshold} is written to {xpintstatus}.{pithreshold}.
Additionally, {xithreshold} is set to `iprio[i]`.
This automatically raises the interrupt enable threashold.
...
{xret} sets {xithreshold} to {xpintstatus}.{pithreshold}.
...
then access to the new CSRs {spintstatus} and {sithreshold} by S-mode or a lower privilege mode
results in an illegal instruction exception,
except if the hypervisor extension is implemented,
and the conditions for a virtual instruction exception apply,
in which case a virtual instruction exception is raised
...
Additionally, it is checked if the interrupt enable threshold was zero,
indicating operation outside of a handler.

Behavior::

[source]
----
if (xsp != 0) and (xsp.SWAP == 0) and (xstatus.XPP == xsp.MODE) and (xithreshold == 0)
...
Explicitly, {xepc}, {xpintstatus} remain unchanged.
