...
NOTE: As this extension modifies interrupt trap behavior but not keep synchronous exceptions traps,
it should only be used in modes which have separate entry points on traps between
interrupts and synchronous exceptions.
Explicitly, mtvec.mode = 0 (direct) is incompatible.
...
When executing an {ipopxret} instruction the context saved during the trap is restored and a return is executed.
