...
for the corresponding privilege level, in this format:
...
- {IPRIO} is equivalent to what would be reported in {mtopi}.{IPRIO}

In case the {iprio_lowercase} array for major interrupts is read-only zeros,
{mtopsi}.{IPRIO} must be 0 or 255, i.e. the simplified implementation,
where {IPRIO} is one when an interrupt is pending, is not allowed.
...
NOTE: Returning {mithreshold}.{iprio_lowercase} as {IPRIO} field
...
- {SIID} is the two's complement representation of the negated major interrupt identity of `i`
- {IPRIO} is equivalent to what would be reported in {stopi}.{IPRIO}

In case the {iprio_lowercase} array for major interrupts is read-only zeros,
{stopsi}.{IPRIO} must be 0 or 255, i.e. the simplified implementation,
where {IPRIO} is one when an interrupt is pending, is not allowed.
...
NOTE: Returning {sithreshold}.{iprio_lowercase} as {IPRIO} field
allows software to use this priority as threshold for spurious interrupts handlers.
If it would return zero,
software may need special handling of spurious interrupts to prevent writing 0 to the threshold register.
