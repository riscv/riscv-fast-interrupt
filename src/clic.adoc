...
Access to CLIC registers `clicintattr[__i__]`, `clicintip[__i__]`, `clicintie[__i__]`, `cliciprio[__i__]`, and `clicmideleg[__i__]`
...
|===
| `miselect` |  `mireg3` bits |  `mireg3` state            | description
| 0x1400     |   31:0        | RW `clicmideleg[31:0]`      | settings for interrupts 31 through 0
| 0x1401     |   31:0        | RW `clicmideleg[63:32]`     | settings for interrupts 63 through 32
4*^| ...
| 0x147F     |   31:0        | RW `clicmideleg[4095:4064]` | settings for interrupts 4095 through 4064
|===
...
|===
| `miselect` |  `mireg3` bits |  `mireg3` state            | description
| 0x1400     |   63:0        | RW `clicmideleg[63:0]`       | settings for interrupts 63 through 0
| 0x1402     |   127:64      | RW `clicmideleg[127:64]`     | settings for interrupts 127 through 64
4*^| ...
|===
...
It increases the number of internal interrupts in S-mode,
and if the H extension is present, in HS- and VS-mode.
...
----
 ...
 (NEW) 0x???   sithreshold  Interrupt enable threshold
----
...
=== Indirect Access HS-mode CLIC interrupt CSRs

In addition to the access to S-mode interrupt CSRs,
HS-mode can access the interrupt delegation array.

If an interrupt _i_ is not present in the hardware, the corresponding CLIC register
locations appear hardwired to zero.

In HS-mode, if an interrupt _i_ is not accessible to HS-mode, the corresponding CLIC register
locations appear hardwired to zero.

==== `clichideleg[__i__]`

In this `hiselect` offset range:

* When HSXLEN = 32, each `hireg3` register controls the interrupt delegation of thirty-two interrupts.

[%autowidth]
|===
| `hiselect` |  `hireg3` bits |  `hireg3` state            | description
| 0x1400     |   31:0        | RW `clichideleg[31:0]`      | settings for interrupts 31 through 0
| 0x1401     |   31:0        | RW `clichideleg[63:32]`     | settings for interrupts 63 through 32
4*^| ...
| 0x147F     |   31:0        | RW `clichideleg[4095:4064]` | settings for interrupts 4095 through 4064
|===

* When HSXLEN = 64, only the even-numbered registers exist and each `hireg3` register controls the interrupt delegation of sixty-four interrupts.

Each interrupt has an associated interrupt privilege mode delegation as defined in the AIA specification.
For the first 64 interrupts, the clichideleg registers mirror the values of hideleg bits in the AIA specification.

=== Indirect Access VS-mode CSRs

If an interrupt _i_ is not present in the hardware, the corresponding CLIC register
locations appear hardwired to zero.

In VS-mode, if an interrupt _i_ is not accessible to VS-mode, the corresponding CLIC register
locations appear hardwired to zero.
...
* When VSXLEN = 32, each `vsireg` register controls the clic priority setting of four interrupts
and each `vsireg2` register controls the clic attribute setting of four interrupts.
All attributes are read-only in VS-mode.

[%autowidth]
|===
| `vsiselect`  |  `vsireg2` bits |  `vsireg2` state         | description

| 0x1000+_i_   |   7:0        | R   `clicintattr[_i_*4+0]` |  setting for interrupt _i_*4+0
| 0x1000+_i_   |  15:8        | R   `clicintattr[_i_*4+1]` |  setting for interrupt _i_*4+1
| 0x1000+_i_   |  23:16       | R   `clicintattr[_i_*4+2]` |  setting for interrupt _i_*4+2
| 0x1000+_i_   |  31:24       | R   `clicintattr[_i_*4+3]` |  setting for interrupt _i_*4+3
|===

* When VSXLEN = 64, only the even-numbered registers exist and each register controls the clic attribute setting of eight interrupts.

==== `cliciprio[__i__]`

In this `vsiselect` offset range:

* When VSXLEN = 32, each `vsireg3` register controls the clic priority setting of four interrupts.
Priority settings are read-only

[%autowidth]
|===
| `vsiselect` |  `vsireg3` bits |  `vsireg3` state             | description

| 0x1000+i   |   7:0          | R   `cliciprio[__i__*4+0]` |  setting for interrupt __i__*4+0
| 0x1000+i   |  15:8          | R   `cliciprio[__i__*4+1]` |  setting for interrupt __i__*4+1
| 0x1000+i   |  23:16         | R   `cliciprio[__i__*4+2]` |  setting for interrupt __i__*4+2
| 0x1000+i   |  31:24         | R   `cliciprio[__i__*4+3]` |  setting for interrupt __i__*4+3
|===

* When VSXLEN = 64, only the even-numbered registers exist and each register controls the clic priority setting of eight interrupts.

==== `clicintip[__i__]` and `clicintie[__i__]`

In this `vsiselect` offset range:

* When VSXLEN = 32, each `vsireg` register reflects the interrupt pending of thirty-two interrupts.
and each `vsireg2` register controls the interrupt enable of thirty-two interrupts.

[%autowidth]
|===
| `vsiselect`  |  `vsireg` bits |  `vsireg` state        |  `vsireg2` bits |  `vsireg2` state         | description
...
* When VSXLEN = 64, only the even-numbered registers exist and each `vsireg` register reflects the interrupt pending of sixty-four interrupts and
each `vsireg2` register controls the interrupt enable of sixty-four interrupts.
...
----
       Number  Name         Description
 (NEW) 0x???   vsclaimi     Claim top interrupt
 (NEW) 0x???   vsithreshold Interrupt enable threshold
----

==== Claim top interrupt (vsclaimi)

Register {vsclaimi} has the same value as {vstopi}.
When this value is not zero,
reading {vsclaimi} has the simultaneous side effect of clearing the pending bit for the reported interrupt identity,
if possible.
Writes to {vsclaimi} are ignored.

==== Interrupt enable threshold (vsmithreshold)

{vsithreshold} is a WLRL register that determines the minimum interrupt priority (maximum priority
number) for an interrupt to be to be considered enabled.
Register {vsmithreshold} implements exactly IPRIOLEN bits,
and thus is capable of holding all priority numbers from 0 to latexmath:[${{2}^{\textrm{IPRIOLEN}} - {1}}$].
When {vsithreshold} is a nonzero value P,
interrupt sources with priority numbers P and higher behave as though those sources were not enabled,
regardless of the settings of their interrupt-enable bits.
...
push:
  ...
  sr a0, OFFSET(sp)         # Save a1
  sr a1, OFFSET(sp)         # Save a2
  sr a2, OFFSET(sp)         # Save a3
  sr a3, OFFSET(sp)         # Save a4
  sr a4, OFFSET(sp)         # Save a5
  sr a5, OFFSET(sp)         # Save a6
  sr a6, OFFSET(sp)         # Save a7
  ...
  csrr zero, xtopi          # Clear ip bit of interrupt if possible
finish_push:
...
pop:
  ...
  lr a0, OFFSET(sp)         # Restore a1
  lr a1, OFFSET(sp)         # Restore a2
  lr a2, OFFSET(sp)         # Restore a3
  lr a3, OFFSET(sp)         # Restore a4
  lr a4, OFFSET(sp)         # Restore a5
  lr a5, OFFSET(sp)         # Restore a6
  lr a6, OFFSET(sp)         # Restore a7
  addi sp, sp, FRAMESIZE    # Adjust stack pointer
