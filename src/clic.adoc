...
`clicintip[__i__]` is a read-write bit that can be updated both
...
When the input is configured for level-sensitive input,
each `clicintip[__i__]` may be writable or read-only.
When read-only, `clicintip[__i__]` bit reflects the value of an input signal to the
...
When writable, additionally, the software-writable is applied with a logical-OR.
Only the software-writable bit participates in
the read-modify-write sequence of a CSRRS or CSRRC instruction.

Note: As per the machine-level ISA, this behavior is required for the SEI interrupt (exccode 9).

If `clicintip[__i__]` is writable, a pending interrupt i can be cleared by writing 0 to this bit.

Writable pending bits can also be cleared when a CSR instruction that accesses {nxti} includes a write.
...
a writable pending bit for software vectored interrupts.  Additional detail
on this is described in the {nxti} section.

The value in the `clicintip[__i__]` is undefined
When switching from level-sensitive mode to edge-triggered mode,
`clicintattr[__i__]` has the value of the software-writable bit, ignoring the previous external signal level.
...
If the interrupt is writable, then the pending bit is also zeroed.
...
pending bit if that is writable, so software doesn't need to clear the
pending bit in the service routine.

In contrast, when a software vectored interrupt is selected,
the hardware will not automatically clear a pending
bit. Harware will clear an writable pending bit upon execution
...
edge-triggered or SW-asserted interrupts and clearing `clicintip[__i__]` via indirect CSR access while same privilege mode mstatus.mie is enabled requires `mireg` register state to be part of the interrupt handler's overall context state save/restore.
