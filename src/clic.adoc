...
** For handling trap stack pointer swap, a new CSR, xtscratch is introduced, with the intention of holding the trap stack pointer for each privilege level.
...
* If the interrupt is edge-triggered, clear the ip bit
