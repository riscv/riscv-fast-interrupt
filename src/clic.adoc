...
:ipprio: pass:q[``**__x__**ipprio``]

:mpp: pass:q[``mpp``]
:mpie: pass:q[``mpie``]
:mipprio: pass:q[``mipprio``]

:pipprio: pass:q[``**__x__**pipprio``]
...
{mipreemptcfg}.{preemptmsk} configures the number of least significant bits of iprio masked out for constructing {ipprio}.
The highest legal value of {mipreemptcfg}.{preemptmsk} is IPRIOLEN.
The interrupt preemption priority {ipprio} is defined as follows:

[source]
----
IPPRIO_MASK = 2^(mipreemptcfg.preemptmsk) - 1
ipprio[i]   = iprio[i] & IPPRIO_MASK
----

A nested horizontal preemption will happen if and only if an interrupt i is pending and enabled,
and the preemption priority of that interrupt ipprio[i] excceds (i.e. is numerically smaller) than {mintstatus}.{mipprio}.
...
Bits   Field         Description
 ...
 26:24  (reserved)
 23:16  mpipprio[7:0] Previous interrupt preemption priority
 15:12  (reserved)
...
A new M-mode CSR, `mintstatus`, holds the active interrupt preemption priority for
...
mintstatus fields
 ...
 31:24   mipprio
 23:16   (reserved)
 15: 8   sipprio if ssclic is supported
  7: 0   (reserved)
----
...
{mintstatus}.{mipprio} and {mpintstatus}.{mpipprio} fields reset to the highest possible value IPPRIO_MAX.
Depending on the number of implemented priority bits, this is

[source]
----
IPRIO_MAX  = 2^(IPRIOLEN)
IPPRIO_MAX = IPRIO_MAX & IPPRIO_MASK
----
...
the current value of {mintstatus}.{mipprio} is written to {mpintstatus}.{mpipprio}.
If the trap is taken on an interrupt, {mintstatus}.{mipprio} is updated to `iprio[__i__]`.

NOTE: Synchronous exceptions do not modify the value of {mintstatus}.{mipprio}.

==== Returns from Handlers

The behavior of an {mret} instruction is modified as follows:

In CLIC mode, {mret} sets {mintstatus}.{mipprio} to {mpintstatus}.{mpipprio}.
...
spintstatus
 ...
 26:24  (reserved)
 23:16  spipprio[7:0] Previous interrupt preemption priority
 15:12  (reserved)
...
Interrupt preemption priority IPPRIO_MAX corresponds to regular
...
In CLIC mode, when a trap _i_ is taken,
the current value of {sintstatus}.{sipprio} is written to {spintstatus}.{spipprio}.
If the trap is taken on an interrupt, {sintstatus}.{sipprio} is updated to `iprio[__i__]`.

NOTE: Synchronous exceptions do not modify the value of {sintstatus}.{sipprio}.

==== Returns from Handlers

The behavior of an {sret} instruction is modified as follows:

In CLIC mode, {sret} sets {sintstatus}.{sipprio} to {spintstatus}.{spipprio}.
