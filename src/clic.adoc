...
----
 ...
 (NEW) 0x348   msp          Stack pointer for conditional swap
...
----
 ...
 (NEW) 0x148   ssp          Stack pointer for conditional swap
...
{ssp}) by S-mode or a lower privilege mode
...
----
   ...
   .align 3
   foo:
      mcspsw                       # Conditionally swap in stack pointer.
      addi sp, sp, -FRAMESIZE      # Create a frame on stack.
      ...
      lw s0, OFFSET(sp)
      addi sp, sp, FRAMESIZE       # Free stack frame.
      mcspsw                       # Conditionally swap out stack pointer.
      mret                         # Return from handler using saved mepc.
----
...
----
  ...
  #----
    ...
    # ...
    addi sp, sp, FRAMESIZE    # Reclaim stack space.
    mcspsw                    # Conditionally swap back stack pointer.
    mret                      # Return from interrupt.
  #-----------------------------------------------------------------------
  #-----------------------------------------------------------------------
   handle_exc:
