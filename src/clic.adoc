...
:eidelivery: pass:q[``**__x__**eidelivery``]
:eithreshold: pass:q[``**__x__**eithreshold``]
:eipN: pass:q[``**__x__**eipN``]
:eieN: pass:q[``**__x__**eieN``]
:setie: pass:q[``**__x__**setie``]
:setip: pass:q[``**__x__**setip``]
:ithreshold: pass:q[``**__x__**ithreshold``]
:xtopei: pass:q[``**__x__**xtopei``]
:topi: pass:q[``**__x__**topi``]
:vstopei: pass:q[``**__x__**vstopei``]
:xtopi: pass:q[``**__x__**xtopi``]
:xtopei: pass:q[``**__x__**xtopei``]
:claimi: pass:q[``**__x__**claimi``]
...
== ACLIC extensions
This section gives an overview for the Advanced Core-Local Interrupt
Controller (ACLIC) extensions.

This table provides a summary of the ACLIC extensions.
...
The Ssaclic extension depends on the Ssia extension.

The Advanced Core Local Interrupt Controller (ACLIC) is an interrupt controller defined for
...
|0x20000000 = | Interrupt delivery from an ACLIC
...
In that mode, {eithreshold}, {eipN} and {eieN} do exist independently,
but mirror the corresponding {ithreshold}, {setip}[k], and {setie}[k] of the connected APLIC domain.

Likewise, the {xtopei} registers mirror the {topi} register of the connected APLIC domain.
Specifically, {vstopei} mirrors the {topi} register of the APLIC domain connected to the IMSIC guest interrupt file selected by VGEIN is selected.
...
Additionally, it simlifies the handling of level-sensitive interrupts.
...
When XLEN = 32, each `xireg` register controls the ALIC priority setting of four interrupts.
...
Therefore, the idelivery setting of the APLIC is not needed in ACLIC mode.
...
If the Smstateen extension is implemented, then the bit 53 (ACLIC) in mstateen0 is
implemented. If bit 53 (ACLIC) of a controlling mstateen0 CSR is zero, then
...
== Support for interrupt vector table (Smivt)

This extension adds a new mode for taking interrupts via a dedicated interrupt vector table.
...
table entry for the associated interrupt (table pointed to new {mivt} or {meivt} CSR),
...
----
 ...
 (NEW) 0x???   meivt        External Interrupt-handler vector table base address
----

==== New {mivt} and {meivt} CSR
...
Both {mivt} and {meivt} are WARL XLEN-bit CSRs,
which hold the base address of the interrupt vector table.
The address held in {meivt} is aligned on a 64-byte or greater power-of-two boundary.
Values other than 0 in the low 6 bits are reserved of {meivt} are reserved.
...
==== Smivt Changes to {tvec} CSR Mode
...
== Conditional Stack Pointer Swap extension (Smcspsw)
...
=== Conditional Stack Pointer Swap at Supervisor Level (Sscsps)
...
[%autowidth]
|===
| Register | synchronous exception | Major Interrupt | External Interrupt (when in interrupt vector table mode)
| a0       | xcause (equal to exccode) | Major identity | Minor Identity
...
push:
  ...
  csrr a5, mintstatus       # Get mintstatus of interrupt context
  sr a4, OFFSET(sp)         # Save xepc
  sr a5, OFFSET(sp)         # Save mintstatus
...
handle_exc:
  ...
  csrr a3, xtinst           # Get mintstatus of interrupt context
...
pop:
  ...
  lr a5, OFFSET(sp)         # Restore mintstatus
  csrw xepc, a4             # Write xepc back to CSR
  csrw mintstatus, a5       # Write mintstatus back to CSR
