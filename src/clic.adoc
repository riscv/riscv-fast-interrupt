...
:xintstatus: pass:q[``**__x__**xintstatus``]
...
the current value of {xintstatus}.{xnipprio} is written to {xpintstatus}.{xpnipprio}.
If the trap is taken on an interrupt, {xintstatus}.{xnipprio} is updated to `iprio[__i__]`.

NOTE: Synchronous exceptions do not modify the value of {xintstatus}.{xnipprio}.

==== Returns from Handlers

The behavior of an {xret} instruction is modified as follows:

{xret} sets {xintstatus}.{xnipprio} to {xpintstatus}.{xpnipprio}.
...
The behavior is assuming Smnip is present. Otherwise, xintstatus will not be saved.
...
The behavior is assuming Smnip is present. Otherwise, the xintstatus CSR will not be restored.
