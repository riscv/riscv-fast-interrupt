...
=== Separating stack per privilege mode

The stack separation per privilege mode is controlled through the {insert CSR here}.xspsw field.
If enabled, the xcspw instruction can be used to atomically swap the stack pointer with the xscratch register if a mode change happened.

This mechanism can also be used to initialize the sp value of a less privileged mode.
If an operating system manages separate stacks per software thread in a lower privileged mode, this mechanism can also be used.

=== Separating stack for interrupts
...
The interrupt stack pointer at a given privilege level has to be initialized by writing to the xtscratch CSR.
After a trap, the xcspw instruction will swap the previous sp with the value in xtscratch if {insert CSR here}.xtspsw is asserted.

=== xcspsw

The xcspsw instruction conditional swaps the sp register with the xscratch register.
...
[source]
----
if ((xtspsw == 1) and (xil != xpil))
{
  tmp = sp
  sp = xtscratch
  xtscratch = tmp
}
elif ((xspsw == 1) and (PRIV != xpp))
{
  tmp = sp
  sp = xscratch
  xscratch = tmp
...
== Smip Interrupt push and pop extension
...
Interupt latency and jitter is reduced by allowing the context save to happen speculatively in parallel to the vector table and handler fetch.
...
Assmed general changes not covered here:
...
** This would fix the issues discovered earlier where mcause in the trampoline is overwritted before reading it in the handler
...
Interrupts shall not enabled before completing the context save/restore operations of the ipush instruction.
...
32: reserved (5) | xpp (2) |  xpie (1) |  xpil (8) | reserved (4) | xpexccode (12)
...
60: reserved (5) | xpp (2) |  xpie (1) |  xpil (8) | reserved (4) | xpexccode (12)
...
#TODO: Define what happens if there is a break point at the ipush instruction address
