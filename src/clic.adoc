...
If an exception trap occurs during the interrupt handler function address fetch,
the {epc} value of the second trap is set the the pc at which interrupt traped.
...
As long as synchronous exceptions can occur during the vector table fetch,
no side-affects of the trap may be executed that prevent resuming of operation after the handling of the synchronous exception.
In particular, {status} and {pintstatus} may not have been modified.

NOTE: Resuming after a fault on a vector table fetch is currently only seen as useful for instruction page faults.

==== smclichv Changes to {epc} CSRs
