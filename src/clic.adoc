...
access to the new CSRs ({stvt}, {sintthresh},
...
If the trap handler function address fetch is unsuccessful and an exception trap occurs,
the {inhv} bit in {pintstatus} of the exception handler privilege mode is set.
On any other trap, the {inhv} bit is cleared.

Setting the {inhv} bit indicates that the corresponding {epc} value
contains a trap handler function address instead of the virtual address of an instruction.

The {inhv} bit can be written by software, including
...
If there is an access exception on the table fetch,
{tval} is either set to zero or written with the faulting address.
...
If the {inhv} is set, the behavior when returning from an {ret} instruction is modified as follows:
as follows:

The {inhv} bit is cleared.
Instead of resuming execution from {epc},
the hart resumes the trap handler memory access to retrieve the function
...
privilege mode's {epc} with the low bits of the address cleared to
