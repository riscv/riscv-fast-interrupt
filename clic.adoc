...
selectively hardware vectored or that the system is not in a CLIC
mode, or returns the address of the entry in the trap handler table
for software trap vectoring.
...
--
 ...
 if (clic.priv==M && clic.level > mcause.pil
   ...
   // No interrupt or a selectively vectored interrupt, or in non-CLIC mode.
   rd = 0;
 }
--
...
== CLIC and PCI Message-Signalled Interrupts (MSI)w
