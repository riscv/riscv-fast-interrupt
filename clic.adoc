...
The following code can be used as the background code that puts the
processor to sleep with no active context, waiting for an interrupt
...
== Managing Interrupts Across Privilege Modes
...
higher-privileged handler.  The RISC-V privileged architecture
provides the {scratch} register to hold this information for a
higher-privilege mode while executing in a lower-privilege mode.  For
the following discussion and code examples, the assumption is that
{scratch} is used to hold the stack pointer but other software
conventions are possible (e.g., {scratch} points to a thread context
block).

=== Interrupt Stack Conventions
...
if transitioning back to the lower-privilege mode.  A special CSR
address is used to provide this {scratch} swap functionality, and is
used once at the entry to a handler routine and once at handler exit.

NOTE: To perform this check in software would require a free register
to read the value from {cause}, but some safe location is required to
save a register for handler's use before the correct stack is known.

=== Scratch Swap CSR ({scratchcsw})

When more than one privilege mode is supported, a new CSR {scratchcsw}
is defined for all but the lowest privilege mode to support
conditional swapping of the {scratch} register when transitioning
between privilege modes.  Though designed to be used with `csrrw`
instructions, these CSRs can be accessed with any CSR instruction.
