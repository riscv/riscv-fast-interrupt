...
Implementations can provide access to CLIC registers clicinttrig[i], clicintip[i], clicintie[i], clicintattr[i], clicintctl[i] 
by memory-mapped registers or by using the Indirect CSR Access method (Smcsrind/Sscsrind).  
The clic register accesses method available to software is indicated by the parameters CLICMEMMAPPED and CLICCSRINDIRECT.
...
----
 miselect  |     mireg             |
 offset    |register state         |

  0x0       clicintattr[(XLEN/8)-1:0]
  0x1       clicintattr[2*(XLEN/8)-1:(XLEN/8)]  
  ...
  0x400     clicintattr[(XLEN/8)-1:0]
  0x101     clicintattr[2*(XLEN/8)-1:(XLEN/8)]
  ...
  0x800     clicintip[XLEN-1:0]       
  0x801     clicintip[2*XLEN-1:XLEN]  
  ...
  0x880     clicintie[XLEN-1:0]       
  0x881     clicintie[2*XLEN-1:XLEN]  
  ...
  0x900     clicinttrig[0]
  0x901     clicinttrig[1]
  ...
  0x91F     clicinttrig[31}
  0x920     reserved for smclicconfig extension
----
...
=== CLIC Register Access via Memory Mapped Registers
...
CLICMEMMAPPED    0-1                           Implements memory-mapped access to clicintip/ie/attr/ctl/trig?                              
CLICCSRINDIRECT  0-1                           Implements indirect CSR access to clicintip/ie/attr/ctl/trig?
----
