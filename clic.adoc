...
WFI will wake up only if an interrupt is locally enabled with `clicintie[__i__]`
...
the same as the current privilege mode and its interrupt level is above or
equal to the current interrupt-level threshold ({intthresh}). This is because
