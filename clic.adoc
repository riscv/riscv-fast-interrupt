...
= RISC-V Core-Local Interrupt Controller (CLIC) Version 0.9-draft-20191120
...
original basic model with either non-vectored or vectored transfer to a handler
...
=== CLIC versus Original Basic Controller
...
modes instead of the original basic modes.  Platform profiles may
...
as the original basic mode, but different than the PLIC.
...
If `nvbits` = 0, then selective interrupt hardware vectoring is not implemented.
So all interrupts are non-vectored and are directed to the common code
at {tvec} register.


If `nvbits` = 1, then selective interrupt hardware vectoring is implemented.
...
NOTE: This behavior significantly reduces
the hardware cost because it only needs to select one global maximum interrupt
and compare with the threshold of the associated mode (while ignoring thresholds
in other modes). Otherwise, hardware would have to select multiple maximum
interrupts (one per mode), compare and qualify with their associated thresholds,
...
and occupies one byte in the memory map for ease of access. This is bit is read-write
and enable/disable the corresponding interrupt.
...
The 2-bit `trig` field specifies the trigger type and polarity for each
...
The 2-bit `mode` field specifies which privilege mode this interrupt
...
the low 16 bits of the {ip} and {ie} registers, so up to 4080 local
external interrupts can be added.

A fixed parameter of the CLIC (`CLICINTCTLBITS`) is how many total bits
are present in the `clicintctl` registers , with 0 {le}
...
`clicintctl` to form an unsigned value, then picks the global maximum
across all pending-and-enabled interrupts based on this value.
Next, the `cliccfg` setting determines how to split
the maximum `clicintctl` value into interrupt level and interrupt
...
WARNING: Selecting an interrupt at a high privilege mode masks any
...
NOTE: When upgrading an earlier original basic interrupt system design
...
original basic mode, the behavior is intended to be software
...
----
 ...
 (NEW) 0xm??   mintstatus   Current interrupt levels
...
When in original basic mode, the {status} register behavior is unchanged
(i.e., backwards-compatible with original basic mode).  When in CLIC mode,
the ``**__m__**pp`` and ``**__m__**pie`` in {status} are now accessible
...
switching between CLIC and original basic modes.
...
The {clicbase} CSR is an XLEN-bit WARL read-write register
providing the base address of CLIC memory mapped registers.
Its value should be set up by the system start-up code to indicate
the starting address of CLIC memory mapped registers. 
Nevertheless, in some low-cost implementation, this CLIC base address
can also be hardwired to a fixed value and thus cannot be changed.
...
These inline handlers can be used with the original basic mode design as
