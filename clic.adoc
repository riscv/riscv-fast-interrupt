...
priv-modes nmbits clicintattr[i].mode V  Interpretation
   ...
   M/S/U      3       00              1  VS-mode interrupt
   M/S/U      3       01              1  Reserved (VU-mode interrupt)
----
...
0x000+8*i       1B/input    RW   vsclictrans[i]
...
----
...
==== Context switching
vsclicint.vmid reduces the number of CLIC memory mapped registers that the hypervisor needs program when changing virtual machines.  Without this field, on each context switch the hypervisor would need to save and restore all implemented vsclictrans registers.
