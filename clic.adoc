...
preempted this instruction.  If one wishes to have horizontal hardware 
vector interrupts preempt the software loop handler, then the ABI for 
the hardware vectored interrupts can be required to save and restore 
mcause. Interrupts will naturally be disabled on entry to that handler. 
This removes the overhead from every iteration of the software-vectored 
loop and places it into the hardware-vectored handler only as needed.
