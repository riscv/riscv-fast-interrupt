...
switches to the handler's privilege mode, sets the hardware vectoring bit {inhv} in {cause} of the handler privilege mode, and clears {status}.{ie}.   At this time, if the associated interrupt pending bit is configured for edge-sensitive input, it is cleared by hardware. The processor then fetches an XLEN-bit handler
...
== CLIC Hypervisor Extension

This is an initial proposal to discuss possible CLIC hypervisor support but will likely be as a future enchancement and not part of 1.0 ratification. 
Possible hypervisor directions for fast interrupts for embedded include: 1) a minimal CLIC extension that adds HS mode as another privilege layer in the stack, so the hypervisor could optionally receive interrupts and optionally enable/hide interrupts from lower privilege modes. This would allow the 2-stage translation and other features of the hypervisor to be used in a CLIC-based system, but would not support sending interrupts directly to descheduled Virtual Machine (VM) contexts (have to route via hypervisor interrupt). 2) support multiple descheduled CLIC contexts to which interrupts could be sent directly while they were sleeping, which would be a much bigger project. 3) add an extension to AIA (instead of working on CLIC) to somehow reduce interrupt latency using the standard hypervisor stack.
 
To support the Hypervisor Extension, additional CSRs and memory mapped registers are provided.

=== Changes to hstatus CSR

hstatus.vgein is ignored in CLIC mode.

=== Changes to Delegation hideleg CSR

The hideleg CSR ceases to have effect in CLIC mode.  The hideleg CSR is still accessible and state bits retain their values when switching between CLIC and original basic interrupt modes.

=== Changes to hvip/hip/hie/hgeip/hgeie/vsip/vsie CSRs

The hie/hgeie/vsie CSRs appear hardwired to zero in CLIC mode, replaced by separate
memory-mapped interrupt enables.

The hvip/hip/vsip CSRs appear hardwired to zero in CLIC mode, replaced by
separate memory-mapped interrupt pendings.

Writes to hvip/hip/hie/hgeip/hgeie/vsip/vsie will be ignored and will not trap (i.e., no access faults).
hvip/hip/hie/hgeip/hgeie/vsip/vsie always appear to be zero in CLIC mode.

In systems that support both original basic and CLIC modes, the state bits in
hvip/hip/hie/hgeip/hgeie/vsip/vsie retain their value when switching between modes.

hstatus.vgein is not used.
hideleg, hvip, hip, hie, hgeip, hgeie, vsip, vsie not used

=== Changes to vscause
Changes to vscause mirror changes to scause for CLIC mode.

----
vscause
 Bits    Field        Description
 XLEN-1 Interrupt     Interrupt=1, Exception=0
    30  vsinhv         Set by hardware at start of hardware vectoring, cleared by hardware at end of successful hardware vectoring
    29  (reserved)
    28  vspp           Previous privilege mode, same as vsstatus.vspp
    27  vspie          Previous interrupt enable, same as vsstatus.vspie
 26:24  (reserved)
 23:16  vspil[7:0]     Previous interrupt level
 15:12  (reserved)
 11:0   exccode[11:0] Exception/interrupt code
----

=== VS-mode CSR changes and additional VS-mode CSRs

vstvec, vstvt, vsscratch, vsepc, vstval, vsnxti, vsintstatus, 
vsintthresh, vsscratchsw, vsscratchcwl are added and behave as described in the CLIC CSRs chapter above.

=== Changes to CLIC Interrupt Attribute (`clicintattr`)

The privileged specification adds a virtualization mode, denoted V, that is useds in conjunction with the privilege mode to determine which of the 5 possible privilege modes (U-mode, HS-mode, M-mode, VU-mode, VS-mode) the hart is operating in.  To support identifying HS-mode interrupts vs. S-mode interrupts, a V field is added to clicintattr.
...
The 2-bit mode WARL field along with the 1-bit v field specifies which privilege mode this interrupt operates in.

----
 Encoding for RISC-V privilege levels (V, Nominal Privilege)

        V P      Name                                  Abbreviation
        1 00     Virtual user mode                     VU-mode
        1 01     Virtual supervisor mode               VS-mode 
        0 00     User mode                             U-mode
        0 01     Hypervisor-extended supervisor mode   HS-mode
        0 11     Machine mode                          M-mode
...
----
priv-modes nmbits clicintattr[i].mode V  Interpretation
       M      0       xx              x  M-mode interrupt

     M/U      0       xx              x  M-mode interrupt
     M/U      1       0x              x  U-mode interrupt
     M/U      1       1x              x  M-mode interrupt

   M/S/U      0       xx              x  M-mode interrupt
   M/S/U      1       0x              x  S-mode interrupt
   M/S/U      1       1x              x  M-mode interrupt
   M/S/U      2       00              x  U-mode interrupt
   M/S/U      2       01              x  S-mode interrupt
   M/S/U      2       10              x  Reserved 
   M/S/U      2       11              x  M-mode interrupt

   M/S/U      3       00              0  Reserved (U-mode interrupt)
   M/S/U      3       01              0  HS-mode interrupt
   M/S/U      3       11              0  M-mode interrupt
   M/S/U      3       00              1  Reserved (VU-mode interrupt)
   M/S/U      3       01              1  VS-mode interrupt
----

Note: The Hypervisor extension of the RISC-V priviledged specification does not currently consider the possibility of U-mode or VU-mode interrupts. 

=== VS-mode CLIC Memory-Mapped Translation Registers

VS-mode CLIC Memory-Mapped Translation Registers are additional fields associated with CLIC Memory-Mapped Registers.  These additional fields are not required to be added to each CLIC Memory-Mapped Register, only CLIC Memory-Mapped Registers that will support interrupts targeted for direct connection to Virtual Machines.  A minimum of 1 VS-mode CLIC Memory-Mapped Register must be implemented.


An implementation defined number of VS-mode Memory-Mapped registers are available.   

VS-translation memory mapped registers are visible to M/HS-mode and appear as hard-wired zeros in VS/U-modes.

VS-mode CLIC-memory mapped registers are visible to M/HS/VS-modes and appear as hard-wired zeros in U-mode.  Not present VS-mode registers appear hardwired to zero.

The base address of the VS-translation register region is specified by the platform specification.

[source]
----
Layout of CLIC VS-translation register fields in M/HS-mode
0x000+8*i       8B/input    RW   vsclictrans[i]
...
----

[source]
----
  vsclictrans register layout

  Bits    Field
  63:46   reserved (WPRI 0)
  45:32   vmid[13:0]
  31:24   vsclicintctrl[7:0]
...
vmid specifies the virtual machine identifier this vs-mode interrupt is associated with.  The number and behavior of implemented vmid bits matches hgatp.vmid bit field behavior as described in the priviledge specification.

vsclicintctrl specifies interrupt level and interrupt priority of vs-mode interrupt.

vsshv is used for selective hardware vectoring.

hgeie is used to pass s-mode interrupts directly to vs-mode. When virtualization mode V is 1, vmid matches hgatp.vmid, and hgeie is 1, vsclictrans.vsip is asserted.

vstrig holds the vsclicintattr.trig value.  It has no hardware function but allows Virtual Machine to program vsclicintattr.trig and the Hypervisor SW to emulate behavior when vsclicintattr.trig doesn't match external hardware.

vsie is the individual vs-mode interrupt enable bit.

vsip is the individual vs-mode interrupt pending bit.  When hgeie is 1, vsip is aliased to clicintip[i].  When hgeie is 0, vsip is a read-write register bit that can be updated both by hardware clearing and by software writes.

vscause_exccode is used to translate an s-mode interrupt to a different virtual address and exccode value. 

In VS-mode, the address of vsclicintip/ie/attr/ctl registers is based on the vsclictrans.vscause_exccode value when vsclictrans.vmid matches hgatp.vmid.  
The VS-mode base address of the VS-mode CLIC region is specified by the platform specification.

M/S/U-mode CLIC Memory-Mapped Registers are not visible to VS-mode and appear hardwired to zero.

[source]
i = vsclictrans.vscause_exccode
----
Layout of VS-mode CLIC regions in VS-mode
0x000+4*i   1B/input    R or RW   vsclicintip[i]
0x001+4*i   1B/input    RW        vsclicintie[i]
0x002+4*i   1B/input    RW        vsclicintattr[i]
0x003+4*i   1B/input    RW        vsclicintctl[i]
...
----

The pending bit of vsclicintip is located in bit 0 of the byte and is an alias of vsclictrans.vsip.  It is read-only when vsclictrans.hgeie is 1 and is RW when vsclictrans.hgeie is 0.

The enable bit of vsclicintie is located in bit 0 of the byte and is an alias of vsclictrans.vsie.

[source]
----
  vsclicintattr register layout

  Bits    Field 
  7:6     mode - hardcoded to 01 - supervisor mode.   
  5:3     reserved (WPRI 0)
  2:1     trig
  0       shv    R
----

vsclicintattr.mode is hardcoded to 01 because VU-mode interrupts are not yet supported.

vsclicintattr.shv is used for selective hardware vectoring.

vsclicintctl specifies vs-mode interrupt level and interrupt priority of vs-mode interrupts.

=== CLIC Hypervisor Interrupt Operation
This section describes the operation of CLIC hypervisor interrupts.

==== Hypervisor Interrupt Overview
As per the Hypervisor extension in the RISC-V priviledged specification, VS-mode interrupts are globally disabled when executing in U-mode.

VS-mode pending interrupts with vsclictrans.vmid matching VMID (hgatp.vmid) are routed to a separate VS-mode priority encoding tree that uses vsclicintattr.mode, vsclicintctrl, vsclictrans.vscause_exccode to determine priority.  This allows VS-mode to control VS-mode priority.

VS-mode pending interrupts when vsclictrans.vmid does not matching VMID or when VMIDLEN = 0 are treated as standard S/HS-mode interrupts with the priority determined by sclicintattr.mode,sclicintctrl, and interrupt number i.  This routes descheduled CLIC context interrupts to HS-mode with a priority controlled by HS-mode. 

==== Context switching
vsclictrans.vmid reduces the number of CLIC memory mapped registers that the hypervisor needs program when changing virtual machines.  Without this field, on each context switch the hypervisor would need to save and restore all implemented vsclictrans registers.

These CSRs need to be saved and restored on context switches.
[source]
----
vsstatus
vstvec
...
vsintthresh
----
as well as any additional s-mode/u-mode CSRs that vs-mode has access to.

==== Virtualization of interrupt facilities for Virtual Machines

As with AIA extension, to give software that runs in a virtual machine the appearance of executing on a real machine
that implements the CLIC extension at supervisor level, responsibility is shared
between hypervisor software and the hardware facilities described in this chapter. While some behaviors
can be handled directly by hardware, others require significant emulation by the hypervisor,
sometimes with hardware assistance.

===== VS-level external interrupts
Interrupts targeted for direct connection to Virtual Machines are configured as S-mode interrupts with the clicintattr.V set.  

The hypervisor SW configures a clicintattr[i] register connected to the external interrupt as follows:
[source]
----
clicintattr[i].mode = HS-mode (S-mode Nominal Priviledge)
clicintattr[i].v = 1
----

and the hypervisor SW configures the external interrupt settings to match the actual hardware:
[source]
----
clicintattr[i].trig to match external interrupt requirement
----

The hypervisor SW configures a vsclictrans[i] register as follows:
[source]
----
vsclictrans[i].hgeie = 1 
vsclictrans[i].vscause_exccode = cause code expected by VM
vsclictrans[i].vmid = VM ID
----

The hypervisor SW enables the external interrupt
[source]
----
clicintie[i] = 1
----
and can switch to the VM machine.


The Virtual Machine configures vsclicint registers as if it is running in supervisor mode on real hardware:
[source]
----
should it be allowed to program the other settings even if it has no effect like edge/level?
vsclicintattr.shv
vsclicintctrl
vsclicintie
----

When an external interrupt clicintip[i] is pending:
if clicintattr[i].mode is set to s-mode, clicintattr[i].v is set, vsclictrans[i].hgeie is set, vsclictrans[i].vmid matches VMID, then the hardware sets vsclictrans[i].vsip and the interrupt is treated as a vs-mode interrupt using the vsclicintie/ctrl settings.  

When an external interrupt clicintip[i] is pending:
If clicintattr[i].mode is set to s-mode and 
clicintattr[i].v is not set OR vsclictrans[i].vmid does not match VMID, the interrupt is treated as a HS-mode interrupt.

If vsclictrans[i].hgeie is not set, hypervisor software can write to vsclictrans[i].vsip to generate virtual pending interrupts.

There is a separate vs-mode priority encoding tree that uses vsclicintip,vsclicintie,vsclicintctrl to determine the highest priority vs-mode interrupt.

If there are no pending and enabled M-mode or HS-mode interrupts and if there are pending and enabled vs-mode interrupts, the vs-mode priority encoding hardware determines the highest priority vs-mode interrupt and the hart takes the interrupt in vs-mode.  

====== When External interrupt edge/level configuration matches Virtual Machine expected configuration
If the external interrupt is edge and a hw method like snxti or hw vectoring is expected to clear the interrupt, hardware will clear the clicintip[i] bit.
If the external interrupt is level, the Virtual Machine will be able to clear the interrupt at the source (with hypervisor hardware address translation) which will clear the clicintip[i] bit.

====== When External interrupt edge/level configuration does not match Virtual Machine configuration
The hypervisor SW will have to emulate clearing the pending interrupt by guest-page faults.

===== Interrupts at VS-mode
The hypervisor SW can also use the vsclictrans registers to emulate vs-mode interrupts.
The hypervisor SW configures a vsclictrans[i] register as follows:
[source]
----
vsclictrans[i].hgeie = 0 
vsclictrans[i].vscause_exccode = cause code expected by VM
vsclictrans[i].vmid = VM id
----

====== VS-mode Edge-triggered Interrupts
HW clearing methods or Virtual Machine writes to vsclicintip[vscause_exccode] will clear VS-mode pending interrupts.  Virtual Machine writes to vsclicintip[vscause_exccode] can also set a pending interrupt.

====== VS-mode Level-triggered Interrupt emulation
Since the Virtual Machine will try to clear the pending interrupt at the source, the hypervisor SW will have to emulate clearing the pending interrupt by guest-page faults.

====== When More Virtual Machine Interrupts are required than HW has resources to support
The CLIC extension doesn't limit the number of virtual machines but performance of context switching will be affected when there are more Virtual Machine interrupts than the HW has resources to support.  

If there aren't enough vsclictrans registers to support all the virtual machines, hypervisor would need to save and restore contexts.  For example, if there is only 1 vsclictrans register, each emulated interrupt would need to be setup by hypervisor.  If not enough vsclictrans registers, need all vsclicint accesses to cause guest-page faults so reads/writes can be emulated by hypervisor.
