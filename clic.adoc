...
...
    MRET =>  {
      ...
      if (minhv) {
        let mepc_inhv = mepc & inhv_pc_alignment_mask(); /* align xepc value to XLEN/8 byte boundary by ignoring low-order bits. */
        if (check_fetch_permissions(mepc_ihnv) = Addr_OK) {
            next_pc = mem_read(mepc_inhv) & ~1; /* mepc_inhv contains an address of a table entry */
        } else {
        ...
        if (check_fetch_permissions(mepc) = Addr_OK) {
          next_pc = mepc & ~1;
        } else {
          /* other side effects of taking a trap */
        }
