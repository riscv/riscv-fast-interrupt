...
{scratch} holds the stack pointer for the higher-privilege mode,
otherwise, {scratch} holds zero.
...
once at the entry to a handler routine and once at handler exit.
Though designed to be used with `csrrw` instructions, these CSRs can
be accessed with any CSR instruction.
