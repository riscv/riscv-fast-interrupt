...
...
    (_, CTL_MRET())  => {
      ...
      if  get_xinhv_value(cur_privilege) & (cur_privilege != prev_priv) then {
        /* xepc contains an address of a table entry instead of the address of an instruction */
        /* recovery scheme does not apply to horizontal exceptions */
        match ext_control_check_pc(xepc) {
          ...
          Ext_ControlAddr_OK(xepc) => {
            /* If successful, clear inhv of current privilege */
            clear_inhv(cur_privilege)
            next_pc = mem_read(xepc) & pc_alignment_mask();
          }
        } else {
          /* xepc becomes next_pc */
          next_pc = xepc & pc_alignment_mask();
        }
      }
    },
...
...
function prepare_xret_target(p) =
 ...
 function get_xinhv_value(p) =
  match p {
    ...
    Supervisor => if (~ (haveSupMode())) then 0 else scause.SINHV(),
    User       => if (~ (haveUsrMode()) | ~(sys_enable_next())) then 0 else ucause.UINHV();      
  }
----
...
inter-hart interrupt signaling.  An additional separate CLIC software
interrupt bit (`csip`) is added to provide support for a local
background interrupt thread, e.g. context switching.
