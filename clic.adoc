...
== CLIC register access via memory map or indirect CSR Access

Clic extensions Smclicindir/Ssclicindir and Smclicmemmap/Ssclicmemmap are specified to provide access to CLIC registers clicinttrig[i], clicintip[i], clicintie[i], clicintattr[i], clicintctl[i] 
by memory-mapped registers or by using the Indirect CSR Access extension method (Smcsrind/Sscsrind).  
Implementations with only indirect CLIC register access, only memory-mapped CLIC register access, or both indirect and memory-mapped CLIC register access are allowed.

== Smclicindir CLIC Register Access via Indirect CSR Access 
...
----
 ...
 offset    |     bits      |  register state        |
  ...
  0x1*i          7:0         R or RW   clicintip[i]
  0x1*i         15:8         RW        clicintie[i]
  0x1*i         23:16        RW        clicintattr[i]
  0x1*i         31:24        RW        clicintctl[i]
  ...
  0xFFF          7:0         R or RW   clicintip[4095]
  0xFFF         15:8         RW        clicintie[4095]
  0xFFF         23:16        RW        clicintattr[4095]
  0xFFF         31:24        RW        clicintctl[4095]
----

----
 miselect  |     mireg2     |  mireg2                |
 offset    |     bits       |  register state        |

  0x0            31:0          RW        reserved for smclicconfig extension 
  0x10           31:0          RW        clicinttrig[0]
  0x11           31:0          RW        clicinttrig[1]
  0x12           31:0          RW        clicinttrig[2]
...
  0x2D           31:0          RW        clicinttrig[29]
  0x2E           31:0          RW        clicinttrig[30]
  0x2F           31:0          RW        clicinttrig[31]
----

----
 miselect  |     mireg6    |  mireg6                |mireg5    |  mireg5                |mireg4    |  mireg4                |mireg3    |  mireg3                |
 offset    |     bits      |  register state        |bits      |  register state        |bits      |  register state        |bits      |  register state        |

 0x0            31:0         RW clicintie[127:96]    31:0       RW clicintie[95:64]      31:0        RW clicintie[63:32]     31:0        RW clicintie[31:0]
...
 0x19           31:0         RW clicintie[4095:4064] 31:0       RW clicintie[4063:4032]  31:0        RW clicintie[4031:4000] 31:0        RW clicintie[3999:3968]       
----
...
== Ssclicindir CLIC Register Access via Indirect CSR Access 
...
----
 siselect  |     sireg6    |  sireg6                |sireg5    |  sireg5                |sireg4    |  sireg4                |sireg3    |  sireg3                |
 offset    |     bits      |  register state        |bits      |  register state        |bits      |  register state        |bits      |  register state        |
...
== Smclicmemmap - CLIC Register Access via Memory Mapped Registers
...
(`clicintip[__i__]`). 
With memory-mapped accesses, it occupies one byte in the memory map for ease
of access.  The pending bit is located in bit 0 of the byte.  
...
With memory-mapped accesses, i occupies one byte in the memory map for ease of access.
With memory-mapped accesses, the enable bit is located in bit 0 of the byte. Software should assume clicintie[i]=0 means no interrupt enabled, and clicintie[i]!=0 indicates an interrupt is enabled to accommodate possible future expansion of the clicintie field.
...
implemented. If bit X of a controlling mstateen0 CSR is zero, then
...
----
  ...
  located at MCLICBASE if smclicmemmap is supported
...
----
  ...
  located at SCLICBASE if ssclicmemmap is supported
...
----
  ...
  located at UCLICBASE if suclicmemmap is supported
