...
= RISC-V Core-Local Interrupt Controller (CLIC) Version 0.9-draft-20210217
...
In case future extensions expand the bit field, from FW perspective 
clicintip[__i__]=zero means no interrupt pending, and 
clicintip[__i__]!=0 (not just 1) indicates an interrupt is pending.
...
level `L` = max({intstatus.xil,xintthresh.th}; the CLIC `priv`,`level`, and `id` fields
...
The overall WFI behavior is summarized in the following table: the Current
`p/ie/l` fields represent the current privilege mode `P` (not
software visible), interrupt enable in {status} `ie` and interrupt
level `M` = max({intstatus.xil,xintthresh.th}, 'iL' = {intstatus.xil}; 
the CLIC `priv` and `level` fields
represent the highest-ranked interrupt currently present in the CLIC
with `nP` and 'nL' representing the pending privilege mode and interrupt level;

 Current  |      CLIC             |->   
 p/ie/l   | priv level            |->   
 P  ?  M  | nP<P   M==0,  nL==0   |-> # no requirement to wake up
 P  ?  M  | nP<P   M==0,  nL!=0   |-> # WFI must wake up
 P  ?  iL | nP<P   iL==0, nL!=0   |-> # WFI must wake up
 P  ?  iL | nP<P   iL!=0          |-> # lower-privilege interrupts masked by interrupt priority reduction tree.
 P  ?  ?  | nP<P   nL==0          |-> # no requirement to wake up
 P  ?  ?  | nP=P   nL==0          |-> # no requirement to wake up
 P  ?  M  | nP=P   0<nL<=M        |-> # no requirement to wake up
 P  ?  M  | nP=P   M<nL           |-> # WFI must wake up
 P  ?  ?  | nP>P   nL==0          |-> # no requirement to wake up
 P  ?  ?  | nP>P   0<nL           |-> # WFI must wake up

Note that a higher-mode threshold setting can cause lower-mode interrupts that are 
enabled and above the lower-mode threshold to not wake up a WFI in the higher-mode. 
When a currently pending higher-mode interrupt is below the threshold set in the 
higher mode, the higher-mode interrupt will effectively mask the lower-mode interrupt 
in the interrupt priority reduction tree, but will not cause WFI to exit in the 
higher-mode due to the higher-mode threshold setting. 

When the higher-mode threshold is set to zero, it cannot mask any pending and enabled 
lower-mode interrupts.
