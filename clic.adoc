...
= RISC-V Core-Local Interrupt Controller (CLIC) Version 0.9-draft-20200908
...
For interrupt level, the 4-bit `cliccfg.nlbits` WARL field indicates
how many bits in `clicintctl[__i__]`
are used to encode the interrupt level.  Valid values are 0--8
and interrupt level are located in the most-significant bits of
`clicintctl[__i__]` register.
