...
=== VS-mode CLIC Memory-Mapped Registers

VS-mode CLIC Memory-Mapped Registers behave differently than the m/s/u CLIC Memory-Mapped Registers and cannot be implemented as an alias of the m/s/u physical CLIC memory-mapped registers to a different address range.   
...
[source]
----
Layout of CLIC VS-translation register region in M/HS-mode
...
hgeie and vgein are used to pass s-mode interrupts directly to vs-mode. When mstatus.MPV is 1, vmid matches hgatp.vmid, the exccode of a pending s-mode interrupt matches vgein, and hgeie is 1, vsclicintip[vscause_exccode] is asserted.

vsie is the individual vs-mode interrupt enable bit.

vsip is the individual vs-mode interrupt pending bit.  When hgeie is 1, vsip is read-only.  When hgeie is 0, vsip is a read-write register bit that can be updated both by hardware clearing and by software writes.
...
----
  ...
  5:3     reserved (WPRI 0)
  2       hardcoded to positive-edge
  1       edge (~vsclicint.hgeie)
  0       shv    RW
----

vsclicintattr.edge is hardcoded to 1 when vsclictrans.hgeie is 0 indicating that software or hardware clearing needs to clear the pending interrupt.  
vsclicintattr.edge is hardcoded to 0 when vsclictrans.hgeie is 1 indicating that software needs clear the pending interrupt elsewhere.
...
VS-mode interrupts with vsclictrans.vmid matching hgatp.vmid are routed to a separate VS-mode priority encoding tree that uses vsclicintattr.mode, vsclicintctrl, vsclictrans.vscause_exccode to determine priority.  This allows VS-mode to control VS-mode priority.

VS-mode interrupts with vsclictrans.vmid not matching hgatp.vmid or VMIDLEN = 0 are treated as standard S/HS-mode interrupts with the priority determined by sclicintattr.mode,sclicintctrl, and interrupt number i.  This routes descheduled CLIC context interrupts to HS-mode with a priority controlled by HS-mode. 
...
These VS CSRs need to be saved and restored on context switches.
vsstatus
vstvec
vsscratch
vsepc
