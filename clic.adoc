...
software visible), interrupt enable (xstatus.ie & clicintie[i]) `ie` and interrupt
level `L` max(xintstatus.mil, xintthresh.th); the CLIC `priv`,`level`, and `id` fields
...
for xscratchcsw/xscratchcswl. In contrast, the “pure read” and “pure write” 
...
operand) on xscratchcsw is not defined/reserved. 
...
Again, only the read-modify-write (swap/CSRRW) operation defined for 
xscratchcswl. The implementation of the non CSRRW variants (when 
either rd or rs1 is x0 or when rs1 is an immediate operand)
on xscratchcsw is not defined/reserved. 
