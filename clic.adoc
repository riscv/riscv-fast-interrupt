...
= RISC-V Core-Local Interrupt Controller (CLIC) Version 0.9-draft-20210311
...
=== CLIC Reset Behavior
The general model is RISC-vhas been mandatory reset state is minimized but 
platforms or company policy might add additional reset requirements.  The 
general privileged architecture states that mstatus.mie is reset to zero, 
so coming out of reset interrupts will not be enabled.  For an S-mode only 
