...
The privilege mode of an interrupt is controlled by both `xcliccfg.xlpmode` and `clicintattr[__i__].mode` as described in the Specifying Interrupt Privilege Mode section below.
...
The `mcliccfg` register has four WARL fields, a 1-bit `mlpmode` field,  a 1-bit `slpmode` field,
...
Restricted view of the mcliccfg.slpmode bit appears as the scliccfg.slpmode bit for supervisor level.
...
The 1-bit `xcliccfg.xlpmode` WARL lower privilege mode interrupts field can be set to 0 or 1.  
If `xcliccfg.xlpmode` = 1, then lower privilege mode interrupts are supported.
If `xcliccfg.xlpmode` = 0, then lower privilege mode interrupts are not supported.
This can allow implementations to physically implement fewer `clicintattr[__i__].mode` bits.

For example, in M-mode-only systems, only M-mode exists so we do not
need any extra bit to represent the supported privilege-mode interrupts. In this case,
no physically implemented bits are needed in the `clicintattr.mode`
and thus `mcliccfg.xlpmode` is 0 (i.e., both `mcliccfg.mlpmode` and `mcliccfg.slpmode` can be hardwired to 0).

In M/U-mode systems with N-extension user-level interrupts support, `mcliccfg.mlpmode` can be
set to 0 or 1 and `mcliccfg.slpmode` is read-only 0.  
If `mcliccfg.mlpmode` = 0, then all interrupts are treated as
M-mode interrupts.  If the `mcliccfg.mlpmode` = 1, then a value of 1 in
...
Similarly, in M/S-mode systems, `mcliccfg.mlpmode` can be
set to 0 or 1 and `mcliccfg.slpmode` is read-only 0.
If `mcliccfg.mlpmode` = 0, then all interrupts are treated as
M-mode interrupts.  If the `mcliccfg.mlpmode` = 1, then a value of 1 in
the most-significant bit (MSB) of a `clicintattr[__i__].mode` register
...
In systems that support all M/S/U-mode interrupts, `mcliccfg.mlpmode` and `mcliccfg.slpmode`
can be set to represent supported privilege-mode interrupts.
`mcliccfg.mlpmode` = 0 indicates that all local interrupts are taken in
M-mode.  `mcliccfg.mlpmode` = 1 and `mcliccfg.slpmode` = 0 indicates that only M and S-mode interrupts are supported so 
the `clicintattr[__i__].mode` MSB selects between M-mode
(1) and S-mode (0).  `mcliccfg.mlpmode` = 1 and `mcliccfg.slpmode` = 1 indicates that M, S, and U-mode interrupts are supported so
the two MSBs of each `clicintattr[__i__].mode` register encode the interrupt's privilege
...
----
priv-modes {mlpmode,slpmode} clicintattr[i].mode  Interpretation
       M      0x                 xx               M-mode interrupt

     M/U      00                 xx               M-mode interrupt
     M/U      10                 0x               U-mode interrupt
     M/U      10                 1x               M-mode interrupt

     M/S      00                 xx               M-mode interrupt
     M/S      10                 0x               S-mode interrupt
     M/S      10                 1x               M-mode interrupt

   M/S/U      0x                 xx               M-mode interrupt
   M/S/U      10                 0x               S-mode interrupt
   M/S/U      10                 1x               M-mode interrupt
   M/S/U      11                 00               U-mode interrupt
   M/S/U      11                 01               S-mode interrupt
   M/S/U      11                 10               Reserved
   M/S/U      11                 11               M-mode interrupt
...
(11: machine mode, 01: supervisor mode, 00 user mode). 
...
Bit 1 of {tvec} is controlled by bit 1 of mtvec (i.e., lower privilege
modes see this bit as read-only. Bit 0 and bits[XLEN-1:6] of
lower-privilege mode's {tvec} are unaffected by changes to
corresponding bits of mtvec. These constraints might change if there
are future additions to the CLIC or other new interrupt controller
specs.
...
switches to the handler's privilege mode, sets the hardware vectoring bit {inhv} in {cause} of the handler privilege mode, and clears {status}.{ie}.   At this time, if the associated interrupt pending bit is configured for edge-sensitive input, it is cleared by hardware. The hart then fetches an XLEN-bit handler
