...
= RISC-V Core-Local Interrupt Controller (CLIC) Version 0.9-draft-20210406
...
software is expected to use {xnxti} to clear the pending bit in edge-triggered mode.
Additional detail on this is described in the {xnxti} section.
...
.WFI is required to wake up due to an interrupt i if interrupt i 
...
* the interrupt priority reduction tree selects interrupt i as the maximum across all pending-and-enabled interrupts and 
* the interrupt i level is greater than max({intstatus}.{il}, {intthresh}.`th` )
...
NOTE: Interrupt i level is a function of CLICINTCTLBITS, cliccfg.nlbits, and `clicintctl[__i__]`
...
NOTE: If there is a non-zero level supervisor interrupt pending and a level-zero machine interrupt pending, the machine interrupt will be the global maximum across all pending-and-enabled interrupts but interrupt level 0 implies no interrupt. So a level-zero pending machine interrupt will mask any lower privilege interrupts.  For this reason, it is not recommended to set interrupt levels to 0.
