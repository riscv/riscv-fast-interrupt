...
...
      ...
      mstatus.MPIE    = 1;
      cur_privilege   = mstatus.MPP;
      ... /* additional standard MRET behavior */

      let xepc = prepare_xret_target(Machine);
      
      if  get_xinhv_value(cur_privilege) & /* check if resuming hardware vectoring */
          (cur_privilege != prev_priv)     /* recovery scheme does not apply to horizontal exceptions */
      then {  
        match check_fetch_permissions(xepc) {
          ...
          Addr_OK => { 
            clear_inhv(cur_privilege)      /* If table entry read successful, clear inhv of current privilege */
