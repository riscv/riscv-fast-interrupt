...
or all privilege modes must run in non-CLIC mode.  In the current spec, if both CLIC and CLINT are supported, then bits[5:2] must be zero for both (i.e., the original basic interrupt model *tvec must also be 64-byte aligned). Other settings of bits[5:2] are reserved. Bit 1 of {tvec} is controlled by bit 1 of mtvec (i.e., lower privilege modes see this bit as read-only. Bit 0 and bits[XLEN-1:6] of lower-privilege mode's {tvec} are unaffected by changes to corresponding bits of mtvec. These constraints might change if there are future additions to the CLIC or other new interrupt controller specs.

----
 (xtvec[5:0])  
 mode        Action on Interrupt
 000000      pc := OBASE                              (original non-vectored basic mode)
 000001      pc := OBASE + 4 * exccode                (original vectored basic mode)
